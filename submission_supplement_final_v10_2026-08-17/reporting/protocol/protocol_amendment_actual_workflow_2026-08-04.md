...
The completed workflow was investigator-supervised and AI-assisted:

1. Structured eligibility, report-to-study mapping and extraction rules were fixed in versioned forms and codebooks.
2. AI systems supported record preparation, PDF-grounded extraction, consistency checks and proposed adjudication.
3. The user approved or delegated specified decision packages and authorized survey-scope processing.
...
This process must be reported as `investigator-supervised, AI-assisted, and claim-governed`. It must not be described as independent duplicate human screening, independent full-corpus human PDF verification, or third-reviewer arbitration.

## Independence boundary

`independent_human_status = not_documented`

User authorization and user-delegated adjudication are valid workflow decisions, but they are not equivalent to a second independent reviewer personally reading every report. Historical filenames or status labels containing `human_adjudicated` or `human_locked` are preserved for hash lineage and are not evidence of independent verification.
