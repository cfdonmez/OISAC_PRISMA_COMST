...
Native LaTeX + BibTeX consolidation milestone (2026-03-24):
- Kullanici karariyla journal-track authoring merkezi artik markdown bundle degil, `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/` klasoru oldu.
- Bu turdan once ek bir memory snapshot alindi:
  - `legacy_archive/mb_upd_260324_214833`
- Template hazirlik ve lokal asset sync dalgalari tamamlandi:
  - `legacy_archive/ieee_tpl_260324_141552`
  - `legacy_archive/bib_sync_260324_151443`
  - `legacy_archive/ast_sync_260324_151619`
- LaTeX yonetim klasorune lokalize edilen kritik varliklar:
  - `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/references.bib`
  - `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/OISAC_COMST_review_body.md`
  - `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/figures/`
  - `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/fig_v_1.png`
  - `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/fig_v_2.png`
  - `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/fig_vi_1.jpg`
  - `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/fig_vi_2.jpg`
- Native section migration artik tamam:
...
- Ana compile omurgasi artik native LaTeX:
...
- Citation/BibTeX migration basariyla acildi:
  - `legacy_archive/cit_mig_260324_154015`
  - Section I, IV, VII, VIII icindeki `O-ISAC-xxx` placeholder'lari `\\cite{O_ISAC_xxx}` cagrilarina cevrildi
  - `oisac_review_working.tex` icine `\\bibliographystyle{IEEEtran}` ve `\\bibliography{references}` baglandi
  - build zinciri `pdflatex -> bibtex -> pdflatex -> pdflatex` olarak sabitlendi
- Bibliography hygiene onarimlari uygulandi:
  - bozuk `O_ISAC_199` kaydi tamir edildi
  - birden fazla HTML entity / ampersand / encoding problemi `references.bib` icinde temizlendi
- Guncel compile durumu:
  - en son dogrulanan PDF = `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/oisac_review_working_build_20260324_214338.pdf`
  - sayfa sayisi = `32`
  - son log taramasinda `LaTeX Error`, `undefined reference`, `undefined citation` gorulmedi
- Sayfa sayisi ve stabilite resmi:
  - staging asamasindaki yaklasik `55` sayfalik markdown-turev paket, native LaTeX ve cite zinciri sonrasinda `32` sayfaya indirildi
  - yani teknik derlenebilirlik ciddi bicimde iyilesti, ama journal-fit kisaltma ihtiyaci hala devam ediyor
- Acik kalan ana riskler:
  1. IEEE Photonics Journal page-budget ile mevcut `32 sayfa` arasinda hala buyuk fark var
  2. overfull/underfull box ve buyuk float/table yerlesimleri henuz tam temiz degil
  3. frontmatter, impact statement, graphical abstract brief ve journal-specific submission paketi henuz final degil
- Bu fazin detayli gun notu:
  - `memory-bank/latex_native_bibtex_transition_2026-03-24.md`

Current working hub:
- `manuscript/IEEE-Transactions-LaTeX2e-templates-and-instructions/oisac_review_working.tex`

Most likely next actions:
1. Section bazli ikinci cite-normalization turu ve remaining prose cleanup
2. buyuk tablo/sekil float'larini native journal-fit duzene cekmek
3. IEEE Photonics Journal short-review package icin agresif kisaltma
4. frontmatter + impact statement + graphical abstract brief hazirligi

---

## Onceki Faz Kayitlari

Review transition + IEEE Photonics Journal target (2026-03-24):
- Kullanici karariyla manuscript artik `survey` kimliginden `review article / systematic review` kimligine cekiliyor.
- Edit oncesi review-transition arsivi olusturuldu:
  - `legacy_archive/rev_trans_260324_120018`
- Canonical review-framing edit dalgasi su katmanlarda uygulandi:
  - `drafts/abstract.md`
...
- Bundle sync ayni turda su hub'lara yansitildi:
...
- Uygulanan ana degisiklik:
  - self-referential `survey` dili sistematik olarak `review` / `systematic review` diline cekildi
  - PRISMA korunarak Section III basligi `REVIEW METHODOLOGY` olarak guncellendi
  - Section V-VI bridge dili, Section VIII roadmap framing'i, ve Section IX sonuc dili review kimligine hizalandi
- Hedef dergi artik acikca:
  - `IEEE Photonics Journal`
- Journal-fit hatirlatmalar memory-bank'e yeni not olarak eklendi:
  - `memory-bank/journal_target_ieee_photonics_journal.md`
- Submission-level riskler:
  - tam open-access kanal
  - graphical abstract ilk submission'da hazir olmali
  - contributed page budget = `7 page` eşiği
  - impact statement (<500 karakter) gerekecek
- LaTeX gecisi icin staging dosyasi acildi:
  - `manuscript/ieee_photonics_review_staging.tex`
- Bundle-body turevi de uretildi:
  - `manuscript/current_bundle/OISAC_COMST_review_body.md`
- Staging PDF derleme testi basarili:
  - komut = `pdflatex --shell-escape ieee_photonics_review_staging.tex`
  - cikti = `manuscript/ieee_photonics_review_staging.pdf`
  - durum = compile PASS, fakat bu henuz `IEEEphot.cls` tabanli final submission paketi degil
  - kalan teknik not = markdown kaynakli bazi link-reference warning'leri ve layout roughness mevcut

Freeze-ready memory sync (2026-03-24):
- Kullanici karariyla figure tarafinda ek aktif polish turu acilmadi; mevcut gorsel paketi bu asamada yeterli kabul edildi.
- Whole-manuscript durum bu noktada:
  - abstract + Section I-IX integrated
  - final editorial readthrough tamam
  - release-gate = `PASS`
  - manuscript = freeze-ready
- Bu tur itibariyla kapanan ana paketler:
  - Section VII closeout / integration audit
  - Section VIII closeout + QA refresh
  - abstract + Section IX canonical source acilisi
  - whole-manuscript final editorial readthrough
  - release-gate / bundle / QA metadata refresh
- Bundan sonra aktif zorunlu drafting kalmadi.
- Yalnizca opsiyonel veya ileride yeniden-acilabilecek kalemler:
  1. secili figure polishleri sadece kullanici tekrar isterse
  2. raw-source canonical cleanup (future reuse icin)
  3. Section III archival upstream evidence recovery
  4. submission/package/export hazirligi yalnizca istenirse

Current working hub degismedi:
- `manuscript/current_bundle/OISAC_COMST_current_bundle.md`

Final editorial readthrough pass (2026-03-24):
- Active bundle path uzerinde publication-facing son dil temizligi uygulandi.
- Ozellikle su alanlar iyilestirildi:
  - Section III: kalan `artefacts` dili publication-facing hale cekildi
  - Section VII: `opened ...` ve `application evidence pack` gibi repo-ici ifadeler temizlendi
  - Section VIII: `artefact/current evidence pack/strict_without_upstream/paper_ids` gibi ic-table audit dili daha okuyucu-dostu anlatima donusturuldu
- `analysis/VII_cr_mrg_v1/section_07_camera_ready.md` ve `analysis/VIII_cr_mrg_v1/section_08_camera_ready.md` yeniden bundle katmanlarina sync edildi.
- `review_package/05_release_gate_QA.md`, `manuscript/current_bundle/RELEASE_GATE_QA.md`, ve `analysis/VIII_cr_mrg_v1/section_08_final_QA.md` yeni SHA/word-count ile refresh edildi.
- Guncel hukme gore:
  - release-gate hala `PASS`
  - kalan isler artik yalnizca optional figure polish + istege bagli son dil parlatma turu

Current working hub degismedi:
- `manuscript/current_bundle/OISAC_COMST_current_bundle.md`

Abstract + Section IX integration (2026-03-24):
- Canonical front/back matter acildi:
  - `drafts/abstract.md`
  - `drafts/section_09_conclusion.md`
- Bu iki kaynak review bundle ve current bundle katmanlarina insert edildi:
  - `review_package/01_manuscript_bundle.md`
  - `review_package/COMST_review_bundle_01_manuscript.md`
  - `manuscript/current_bundle/OISAC_COMST_current_bundle.md`
- Boylece integrated manuscript artik `abstract + Section I-IX` kapsaminda okunabiliyor; Section I-F organization akisi ile bundle scope tekrar hizalandi.
- Release-gate hukmu artik:
  - `PASS`
  - yapisal blocker kalmadi
  - kalanlar yalnizca optional figure polish + son editorial refinement seviyesi
- Su anki en mantikli sonraki adim:
  1. secili asset-level visual polish kalemleri
  2. whole-manuscript final editorial readthrough
  3. gerekirse son release-note refresh

Current working hub degismedi:
- `manuscript/current_bundle/OISAC_COMST_current_bundle.md`

Bundle hygiene + release-gate refresh (2026-03-24):
- Section VII closeout sync tamamlandi:
  - `Fig. VII-1` caption/text dili gorselin deployment-map rolune hizalandi
  - `Table VII-2` tekrar 5 macro-domain kapsamina getirildi (`indoor_environments` satiri geri eklendi)
- Section VIII closeout sync tamamlandi:
  - `V / VI / VII -> VIII` bridge dili opener'a acikca eklendi
  - `Fig. VIII-1` icin kisa okuma rehberi metne baglandi
  - `VIII-G` icinde bos discrepancy rows artik schema-preserving placeholder olarak aciklanıyor
- Bundle hygiene/release-gate notlari refresh edildi:
...
- Guncel release-gate hukmu:
  - `CONDITIONAL PASS` korunuyor
  - yapisal blocker artik Section VI placement veya Section VII/VIII closeout degil
  - kalan ana karar = Section IX source/skeleton
  - figure tarafinda kalanlar yalnizca asset-level polish
- Su anki en mantikli sonraki adim:
  1. abstract + Section IX skeleton
  2. gerekirse secili asset-level visual polish
  3. sonra final release-gate revisit

Current working hub degismedi:
- `manuscript/current_bundle/OISAC_COMST_current_bundle.md`

Section II `Fig. II-1` sync + day-end handoff (2026-03-17):
- `Fig. II-1` active-state bugun yeniden teyit edildi; figure referansi su katmanlarda aktif:
  - `drafts/section_02_fundamentals_draft.md`
  - `review_package/01_manuscript_bundle.md`
  - `review_package/COMST_review_bundle_01_manuscript.md`
  - `manuscript/current_bundle/OISAC_COMST_current_bundle.md`
...
- Section II guncel hukmu:
  - `2 fig + 2 table` package halen complete
  - `Fig. II-1` = active, near-final, yalnizca optional visual polish kaldi
  - `Fig. II-2` = active, teknik relation cleanup halen acik
- Whole-manuscript recap bu handoff itibariyla:
  - aktif visual package artik Section I / II / IV / V / VI / VII / VIII katmanlarinda mevcut
  - Section III freeze-ready durumda
  - release-gate aciklari artik missing-core-package degil; cleanup / hygiene / later abstract + Section IX sirasinda
- Yarin icin en mantikli re-entry sirasi:
  1. Section II figure cleanup pass (`fig_ii_2` once, `fig_ii_1` optional polish)
  2. Section VII / VIII local cleanup + bundle hygiene
  3. abstract + Section IX skeleton only after figure/release-gate closeout
- Working hub degismedi:
  - `manuscript/current_bundle/OISAC_COMST_current_bundle.md`

Current flow snapshot (2026-03-15, gun sonu / handoff):
- Whole-manuscript canonical akisi artik su merkez etrafinda okunuyor:
  - `manuscript/current_bundle/OISAC_COMST_current_bundle.md`
- Bu merkez canonical authoring source degil; yalnizca hizli okuma / package / handoff hub'i.
- Section bazli mevcut durum:
  - Section II: `2 fig + 2 table` tamam; kalan is sadece `fig_ii_1` visual polish ve `fig_ii_2` teknik cleanup
  - Section III: freeze-ready
  - Section IV: prose polish + artifact package tamam; `Table IV-D`, `Fig. IV-1`, `Fig. IV-2` aktif
  - Section V: gercek veri-temelli `Fig. V-1 / Fig. V-2` aktif; canonical `221 -> 220` drift kapandi
  - Section VI: section-level tamam; `Fig. VI-1 / Fig. VI-2` aktif, kalan is polish/revision seviyesi
  - Section VII: freeze-ready; `Fig. VII-1` aktif, closeout-audit odakta
  - Section VIII: freeze-ready / monitor-only
- Bu turda kapanan ana isler:
  - Section V denominator reconciliation
  - Section IV table/figure/equation package
  - whole-manuscript figure numbering policy freeze
- Su anki en mantikli sira:
  1. kalan figure polish / placement kalemleri
  2. bundle hygiene / son release-gate consistency
  3. abstract + Section IX skeleton
- Dikkat:
  - Section IV aktif figure'leri canonical filtreli oldugu icin guvenli
  - ama bazi ham Section IV analysis CSV'leri gelecekte yeniden kullanilacaksa canonical filtering yeniden uygulanmali
  - release-gate durumu halen `CONDITIONAL PASS`

Section VI figure review sync (2026-03-15, late update):
- `Fig. VI-1` ve `Fig. VI-2` kullanici tarafinda `current_bundle/figures/` altina kaydedildi ve review edildi.
- Ayrintili duzeltme notu:
  - `.agent/workflows/section_06_figure_review_notes_20260315.md`
- Guncel karar:
  - `Fig. VI-1` = near-final; kucuk visual polish disinda blocker yok
  - `Fig. VI-2` = guclu draft; bir revizyon turu daha gerekli
- Henuz canonical manuscript insertion yapilmadi.
- Dogru sonraki sira:
  1. corrected final assetleri uret
  2. `drafts/fig_vi_1.*` ve `drafts/fig_vi_2.*` altina canonical kopyalari koy
  3. sonra `drafts/section_06_draft.md` icine figure referanslarini ekle
  4. bundle/current_bundle refresh et

Section VI figure integration sync (2026-03-15, late update 2):
- Kullanicinin yeni yonlendirmesiyle review notundan sonra dogrudan integration turu uygulandi.
- Canonical assetler:
  - `drafts/fig_vi_1.jpg`
  - `drafts/fig_vi_2.jpg`
- Insertion yapilan katmanlar:
  - `drafts/section_06_draft.md`
  - `review_package/01_manuscript_bundle.md`
  - `review_package/COMST_review_bundle_01_manuscript.md`
  - `manuscript/current_bundle/OISAC_COMST_current_bundle.md`
- Guncel hukum:
  - Section VI figure placement artik tamam
  - bundan sonraki degisiklikler polish/revision seviyesinde degerlendirilecek

Section VII figure insertion sync (2026-03-15):
- Kullanicinin `fig_vii_1` asseti canonical akisa baglandi.
- Canonical asset:
  - `drafts/fig_vii_1.jpg`
- Insertion yapilan katmanlar:
  - `analysis/VII_cr_mrg_v1/section_07_camera_ready.md`
  - `review_package/01_manuscript_bundle.md`
  - `review_package/COMST_review_bundle_01_manuscript.md`
  - `manuscript/current_bundle/OISAC_COMST_current_bundle.md`
- Guncel yorum:
  - Section VII artik tablo-dominant ama figuresiz degil
  - `Fig. VII-1` aktif
  - sonraki lokal is bu figure icin visual/text cleanup ve Section VII genel closeout-audit

Section VIII figure decision sync (2026-03-15):
- Section VIII icin yeni karar: figure artik optional degil, `mandatory`.
- Yeni closeout + figure plan notu:
  - `.agent/workflows/section_08_closeout_checklist_20260315.md`
- Hedef package:
  - `Fig. VIII-1` = challenge-to-roadmap dependency map
- Sonraki mantikli sira:
  1. Section VII closeout-audit
  2. Section VIII `Fig. VIII-1` generate/placement
  3. sonra abstract + Section IX skeleton

Section VIII prompt variant sync (2026-03-15):
- Ayni not icine `Fig. VIII-1` icin ikinci, daha icon-heavy ve visual-forward prompt varyanti eklendi.
- Bu varyant daha "flagship survey figure" karakteri hedefliyor:
  - daha guclu merkez hub
  - daha zengin ikon kullanimi
  - daha editorial panel kompozisyonu

Section VIII figure insertion sync (2026-03-15):
- Kullanici tarafinda uretilen `fig_viii_1.jpg` canonical akisa baglandi.
- Canonical asset:
  - `drafts/fig_viii_1.jpg`
- Image insertion yapilan yerler:
  - `analysis/VIII_cr_mrg_v1/section_08_camera_ready.md`
  - `review_package/01_manuscript_bundle.md`
  - `review_package/COMST_review_bundle_01_manuscript.md`
  - `manuscript/current_bundle/OISAC_COMST_current_bundle.md`
- `manuscript/current_bundle/FIGURE_STATUS.md` de Section VIII figure'i aktif olarak isaretlendi.
- `analysis/VIII_cr_mrg_v1/section_08_final_QA.md` figurlu current state icin refresh edildi:
  - word count = `5805`
  - SHA256(section_08_camera_ready.md) = `71CD764A779C9E6F7D734D1B5AA1F3F253907BF6454825A57C45C892BB52EFF5`
- Son durum:
  - Section VIII artik aktif `Fig. VIII-1` tasiyor
  - sonraki is bu figure icin visual/text cleanup ve genel Section VIII closeout audit

Day-end handoff snapshot (2026-03-15):
- Bugun kapatilan ana kalemler:
  - Section VI figures active
  - Section VII `Fig. VII-1` active
  - Section VIII `Fig. VIII-1` active
  - Section VIII QA refresh complete (`READY: PASS`)
- Bu kapanista acik kalanlar yeni section drafting degil:
  1. Section II / VII / VIII figure cleanup
  2. Section VII closeout / integration audit
  3. bundle hygiene / final consistency pass
  4. abstract + Section IX skeleton
- Calisma merkezi:
  - `manuscript/current_bundle/OISAC_COMST_current_bundle.md`
- Canonical authoring katmani degismedi:
  - `drafts/`
  - `analysis/`
  - `review_package/`

Section III (Methodology) repo-validated conditionally-freeze-ready duruma getirildi. Section IV icin prose polish ustune artifact-package pass de tamamlandi: `Table IV-D` eklendi, canonical veri-temelli `Fig. IV-1 / Fig. IV-2` uretildi ve bundle katmanina baglandi. Section V icin gercek veri-temelli `Fig. V-1 / Fig. V-2` asset uretimi ve canonical `221 -> 220` denominator uzlasimi tamamlandi. Section VI authoritative recovery merge, core hardening, ve integration pass tamamlandi. Section VII icin survey-alignment, reporting-policy lock, manuscript-level consistency pass, ve `Fig. VII-1` insertion uygulandi. Section VIII editorial closeout, `Fig. VIII-1` insertion, ve QA refresh tamamlandi. Tam manuskript integration pass ve release-gate QA da uygulandi. Whole-manuscript figure inventory cikartildi ve section-bazli figure strategy lock notu olusturuldu; figure numbering policy artik freeze edildi. Aktif manuskript odagi artik kalan figure polish/placement kalemleri, bundle hygiene, ve sonrasinda abstract + Section IX skeleton kaynagidir. Abstract ve Section IX final yazimi ise bilerek sonraya birakildi; bunlar figure paketi ve kalan release-gate tutarliliklari kapandiktan sonra skeleton-draft olarak acilacak.
...
- Yeni aktif global siralama:
  - Section IV graphic / table / equation package
  - Section VI user-owned figure placement
  - bundle hygiene + final release-gate consistency
  - sonra abstract + Section IX skeleton drafting

Section V canonical denominator reconciliation snapshot (2026-03-15):
- Legacy extra paper `O_ISAC_347` Section V plotting/manuscript path'inden cikarildi.
- Canonical Section V omurgasi artik su sayilarla konusuyor:
  - `225` raw scenario points
  - `220` raw papers
  - `53` governed usable points
  - `51` papers with at least one governed usable scenario
- `analysis/V_ev_v2/generate_section5_figures.py` canonical included-corpus filtresiyle guncellendi ve `fig_v_1.png` / `fig_v_2.png` yeniden uretildi.
- Sonuc:
  - Section V legacy denominator uyumsuzlugu kapandi
  - aktif Section V isi artik denominator degil; yalnızca gerekirse son caption/polish seviyesi.

Section IV artifact-package snapshot (2026-03-15):
- Section IV package "planlanan" durumdan cikti ve canonical source'a uygulandi.
- Uygulanan artefaktlar:
  - `Table IV-D`
  - `Fig. IV-1` (`drafts/fig_iv_1.png`)
  - `Fig. IV-2` (`drafts/fig_iv_2.png`)
- Uretim hatti:
  - `analysis/IV_ev_v2/generate_section4_figures.py`
  - canonical filter = `screening/included_studies_canonical.csv`
- Onemli not:
  - aktif figure'ler canonical filtreli oldugu icin guvenli
  - ancak bazi ham Section IV analysis dosyalari (`cluster_map.csv`, `section4B/C/D_evidence.csv`) halen `O_ISAC_347` izi tasiyor; gelecekte dogrudan yeniden kullanilacaksa yeniden canonical filtering zorunlu
- Sonuc:
  - Section IV icin kalan is package-olusturma degil
  - bundan sonra yalnizca optional visual polish / caption refinement / upstream cleanup seviyesi

Section VI figure-data prompt snapshot (2026-03-15):
- Section VI icin yeni veri-beslenmis prompt/spec notu yazildi:
  - `.agent/workflows/section_06_figure_data_prompts_20260315.md`
- Ana karar degismedi:
  - Section VI cekirdek package = `2 figure`
  - `Fig. VI-1` = medium-conditioned enabler landscape
  - `Fig. VI-2` = feasibility-to-deployment systems map
- Prompt dosyasi artik canonical `220` duzeltmesini ve OPA / ORIS / ML strict headline counts'i iceren hazir tasarim brief'i sagliyor.
- Son revizyon:
  - Section VI prompt'lari Section I figure'lerinin daha guclu editorial / panel-estetigine yaklastirildi
  - ozellikle `Fig. VI-2` artik duz corporate pipeline degil, survey-style systems map olarak tasarlanacak

Section VI figure review snapshot (2026-03-15):
- Kullanici tarafinda uretilen `fig_vi_1.jpg` ve `fig_vi_2.jpg` `manuscript/current_bundle/figures/` altina kaydedildi.
- Review notu:
  - `.agent/workflows/section_06_figure_review_notes_20260315.md`
- Guncel hukum:
  - `Fig. VI-1` = near-final; yalnizca kucuk visual polish
  - `Fig. VI-2` = guclu taslak; teknik etiket ve duplicate callout temizligi gerekli
- En kritik duzeltmeler:
  - `OPA micro-mirror array` ifadesi `optical phased array` mantigina gore duzeltilmeli
  - duplicate `ML/AI = 53 study-level papers` callout'u teke dusmeli
  - duplicate `Benchmark contract` etiketleri teke inmeli
  - `Fig. VI-1` icindeki `Total` basligi kaldirilabilir
- Henuz manuscript image insertion yapilmadi; once corrected final assetlerin canonical `drafts/fig_vi_1.*` / `drafts/fig_vi_2.*` kopyalari alinacak.
...
- Net durum:
  ...
  - bu not artik tarihsel; sonraki snapshot'ta canonical denominator uzlasimi da kapanmistir

Section V real-figure generation snapshot (2026-03-15):
- Section V icin AI prompt yaklasimindan vazgecilip veri-temelli plotting hattina gecildi.
- Yeni local script:
  - `analysis/V_ev_v2/generate_section5_figures.py`
- Uretilen gercek assetler:
  - `drafts/fig_v_1.png`
  - `drafts/fig_v_2.png`
- Bundle kopyalari:
  - `manuscript/current_bundle/figures/fig_v_1.png`
  - `manuscript/current_bundle/figures/fig_v_2.png`
- Canonical Section V metnine image insertion yapildi ve bundle katmanina propagate edildi.
- Guncel durum:
  - `Fig. V-1` aktif
  - `Fig. V-2` aktif
  - Section V icin artik acik is "figure var mi?" degil; yalnizca gerekirse caption-level polish ve final whole-manuscript drift-check
