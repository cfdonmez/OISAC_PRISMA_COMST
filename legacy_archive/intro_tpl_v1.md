...
**Pattern D: The "Market-Scale" Hook** (Evidence of Urgency)  
- *Structure:* Lead with growth stats, CAGR, or population/fleet projections to show inevitability and urgency.  
- *Example:* "With a 28.58% CAGR and over one million commercial UAVs projected by 2026, UAV swarms are becoming critical infrastructure targets." (COMST_009, COMST_034)
...
- **Pattern: "Gap Stack (G1-G4) Motivation"** (Security/Privacy Surveys)  
  - *Use when:* Framing security or privacy surveys that must systematize the field.  
  - *Template Gaps:* `Gap 1: Missing vulnerability perspective.` `Gap 2: No unified view of diverse attacks.` `Gap 3: Sparse defenses.` `Gap 4: Lack of formal/theoretical grounding.` (COMST_025)
...
|-----------|------|--------------|--------------|------------|------------|-------------|
| [Ref A]   | 2020 | Yes          | Partial       | Partial    | [Topic A]  | No 6G focus |
| [Ref B]   | 2022 | Partial      | Yes           | Yes        | [Topic B]  | No O-ISAC   |
| **O-ISAC**| 2024 | Yes          | Yes           | Yes        | **Unified**| **Ours**    |

**Pattern C: The "Positioning of Our Survey" Block**  
- *Structure:* A short paragraph after the related-work table explicitly stating why this survey is different (e.g., broader attack surface, simulation-based evidence, dual axes).  
- *Example:* "We select Tor, I2P, and Freenet as cases to generalize security insights and adopt a bottom-up & top-down investigation that previous Tor-only surveys lack." (COMST_025)

**Pattern D: Ontology/Scope Figure Call-out**  
- *Use when:* You define an ontology (e.g., vulnerabilities → attacks → defenses → formal security) to justify scope.  
- *Template sentence:* "Fig. X depicts the ontology guiding our investigation, linking vulnerabilities, attacks, defenses, and formal security across the protocol stack." (COMST_025)
...
- *Common Bullets:*  
    ...
    - **Simulation-Backed Evidence:** "We complement the literature review with **realistic simulations/attack analysis** to quantify impact." (COMST_009)  
    - **Dual-Axis Taxonomy:** "We classify the field along **problem axes** (e.g., channel access, routing, task assignment...) and **method axes** (heuristic/policy design/policy learning/hybrid) to guide algorithm choice." (COMST_034)  
    - **Full Security Lifecycle:** "We systematize **vulnerabilities → attacks → defenses → formal security** to expose root causes and mitigation paths." (COMST_025)
...
### 9. Comparative/Scope Subsections
Increasingly common in security-centric and taxonomy-heavy surveys (COMST_009, COMST_025, COMST_034).  
- **Positioning/Scope:** A dedicated subsection after Related Work that states case selection (e.g., Tor/I2P/Freenet) or scenario boundaries.  
- **Comparisons With Other Works:** An explicit subsection (often with a table) contrasting the survey against recent literature and highlighting distinct contributions (e.g., simulation coverage, ontology, dual axes).
...
- **Analyzed Papers:** COMST_001 to COMST_034.  
- **Validation Status:** Partially validated; extend with ongoing COMST_03x updates.  
- **Key Trend (2023-2024):**  
    ...
    - **Gap Enumerations:** Security/privacy surveys explicitly enumerate Gap 1–4 before listing contributions (COMST_025).  
    - **Market Evidence Hooks:** Early quantitative growth stats justify urgency (COMST_009, COMST_034).  
    - **Dual-Axis Framing:** Contributions often pair domain axes (e.g., six mission types) with method axes (heuristic/design/learning/hybrid) to guide readers (COMST_034).
