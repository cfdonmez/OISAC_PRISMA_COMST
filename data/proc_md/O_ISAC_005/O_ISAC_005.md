...
$$g_o = \left[ \operatorname{erf} \left( \frac{\sqrt{\pi} \gamma}{2\sqrt{2} \phi L_{\text{SD}}} \right) \right]^2 \times h_{\text{o}},$$
 (2)
...
where  $\phi_a$  and  $\phi_e$  are the azimuth and elevation angles of direction. The directional gain of the main lobe is  $\frac{G_0}{(\Lambda_m/2)^2}$  with  $G_0=7500$ . In practice, the antenna power gain  $g_0$  is considered insignificant outside the main-lobe. Thus, the projection of the main-lobe on the ground is a disc of radius  $r_m=h_m\tan(\Lambda_m/2)$ . That is, the coverage cell of the UAV is the projection disc of the main lobe due to the huge gap between the main lobe level and the side lobe level [15].
...
$$(P0): \quad \max_{\Lambda_{\rm m}, \Omega_{\rm m}} \quad \Phi_{\rm m}(\Omega_{\rm m}, \Lambda_{\rm m}) + \Phi_{\rm gE}(\Lambda_{\rm m}) \tag{12}$$

s.t. 
$$\Phi_{\rm m}(\Omega_{\rm m}, \Lambda_{\rm m}) \le C_{\rm SD},$$
 (13)
...
| Receiver diameter              | γ                                                                  | 5 cm          |
| Beam divergence                | φ                                                                  | 2.5 mrad      |
| Responsivity                   | ρ                                                                  | $0.5V^{-1}$   |
...
| Bandwidth                      | $\Omega_{ m m}$                                                    | 50-250 MHz    |
...
Update  $\ell \leftarrow \ell - \frac{\ell}{\mathcal{E}_{t-1}}$ 

employs a state model comprised of  $\varrho=(\Lambda_{\rm UAV},\Omega_{\rm UAV})$ , where  $\Lambda_{\rm UAV}$  is the beamwidth of UAV and  $\Omega_{\rm UAV}$  denotes the bandwidth. The state for a UAV deployment can be denoted as  $\Lambda_{\rm UAV}:\{\Lambda_{\rm min},...,\Lambda_{\rm max}\}$ ,  $\Omega_{\rm UAV}:\{\Omega_{\rm min},...,\Omega_{\rm max}\}$ . The agent carries out an action  $a_t\in {\sf A}$  at each step, which comprises adjusting the beamwidth  $\Lambda$  (increase or decrease) and the bandwidth allocation  $\Omega$  (increase or decrease) based on the decision policy  ${\cal P}$  which is determined the Q-table,  $Q(s_t,a_t)$ . The conditional transition probability  $p(s_{t+1},r_t|s_t,a_t)$  describes the transition from state  $s_t$  to state  $s_{t+1}$  with reward  $r_t$  when action  $a_t$  is taken. Through the use of Q-learning, our goal is to optimize the long-term reward, denoted as  ${\cal Z}_t$ ,
...
$$r_{t} = \begin{cases} 1, & \text{if } \Phi_{\text{UAV,new}} > \Phi_{\text{UAV,old}}, \\ -0.1, & \text{if } \Phi_{\text{UAV,new}} = \Phi_{\text{UAV,old}}, \\ -1, & \text{if } \Phi_{\text{UAV,new}} < \Phi_{\text{UAV,old}}. \end{cases}$$
(18)
...
The plot in Fig. 3 shows the end-to-end UAV throughput  $\Phi_{\rm m}$  and the total throughput of edge users  $\Phi_{\rm m}+\Phi_{\rm gE}$  over UAV's circular trajectory. As expected,  $\Phi_{\rm m}$  significantly changes with the altering UAV backhaul distance during UAV movement. Interestingly, the total throughput of edge users mirrors this trend, suggesting that the UAV's performance largely determines edge user performance, even though the GBS is also serving them. Varying the SNR threshold  $\Theta_{\rm th}$  clearly affects the achievable throughputs. Changes in light backscattering affect the system, influencing overall throughput performance. These findings underscore the importance of optical channel sensing, which plays a crucial role in the
