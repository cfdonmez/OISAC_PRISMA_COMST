...
where  $\gamma = b - a \neq 0$  is the differential symbol sequence, and  $\phi(t;\gamma) = 2\pi h \sum_{i=-\infty}^{\infty} \gamma_i \int_{-\infty}^{t} g\left(\tau - iT_c\right) d\tau$  is the phase difference. Therefore, the minimum square Euclidean distance is given by
