...
$$\eta_r = 1 - \frac{\int_{\theta_c}^{\frac{\pi}{2}} \int_{r_b}^{w_0} E_0^2 \exp(-2\frac{r^2}{w_0^2}) r dr d\theta_i}{\frac{\pi E_0^2 w_0^2}{2} (1 - \exp(-2\frac{r^2}{w_0^2}))}$$
(2)
...
$$\eta_r = \frac{\int_{\theta_c}^{\frac{\pi}{2}} \int_{r_b}^{w_0} E_0^2 \exp(-2\frac{r^2}{w_0^2}) r dr d\theta_i}{\frac{\pi E_0^2 w_0^2}{2} (1 - \exp(-2\frac{r^2}{w_0^2}))}$$
(3)
...
transmission matrix 
$$M_2 = M_{air1}M_gM_{air2}M_{sl2} = \begin{bmatrix} a_g & b_g \\ c_o & d_o \end{bmatrix}$$
.
...
$$\eta_m = \left| \frac{2 \exp(\frac{G^2 - 4FH}{4F})}{w_0 w_s F} \right|^2 \tag{9}$$
...
where the measurement light  $I_{\rm m}=I_0\eta_{\rm m}\eta_r^2$ , reference light  $I_{\rm m}=I_0(1-\eta_r)$ ,  $L_{\rm fiber}$  is the optical path length from SMF trench surface to SMF output surface. The fringe contrast is expressed as follows:
