...
$$h_c = \begin{cases} \frac{(m+1)A_c\rho_c}{2\pi d^2} cos^m(\varphi)\kappa(\theta) \cos(\theta), & \text{for } 0 \le \theta \le \varphi_c \\ 0, & \text{else,} \end{cases}$$
 (1)
...
$$\hat{\tau} = \arg\max\sum_{n=0}^{m} y(n)x(n-\tau) \tag{3}$$
