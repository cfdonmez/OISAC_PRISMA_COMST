...
Boyu Dong <sup>10</sup>, Junlian Jia <sup>10</sup>, Li Tao <sup>10</sup>, Guoqiang Li <sup>10</sup>, Zhongya Li <sup>10</sup>, Changle Huang <sup>10</sup>, Jianyang Shi <sup>10</sup>, *Member, IEEE*, Haipeng Wang <sup>10</sup>, *Senior Member, IEEE*, Zhenzhou Tang <sup>10</sup>, *Member, IEEE*, Junwen Zhang <sup>10</sup>, Shilong Pan <sup>10</sup>, *Fellow, IEEE, Fellow, OSA*, and Nan Chi <sup>10</sup>, *Member, IEEE, Fellow, OSA* 
...
| Waveform | Range resolution(cm) | Data rate (Gbps)  | MMW/THz<br>Gen. Method | Band | Ref        |
|----------|----------------------|-------------------|------------------------|------|------------|
...
| TEDM     | 1 50 4 30            | 15 60 (line)      | DIH                    | W    | This paper |
...
where  $\mathbb{C}$  is the complex up-sampling signal; R(.) and I(.) denote the real and imaginary parts, respectively, and \* denotes the convolution operation. In this paper, the SCM signal exists in the form of TDM, FDM or a mixture of the two, so (4) can be written as

$$x_{com}(t) = \begin{cases} Y_{FDM}(t), 0 < t \le T_{LFM} \\ Y_{TDM}(t), T_{LFM} < t \le T_s \end{cases}$$
 (5)
...
$$R_{SNR} = \left[ \frac{P_t G_t G_r \sigma \lambda^2}{(4\pi)^3 S_{i,\text{min}}} \right]^{1/4} \tag{9}$$
...
$$\Delta f = \frac{B_r}{T_{LEM}} \frac{2\Delta R}{c} \tag{10}$$
...
$$E_{MZM-1}(t) = E_{c1} \exp(j\omega_{c1}t) \cos(m_1 \cos \omega t + \theta/2)$$
 (13)
...
$$E_{OBPF}(t) \propto \begin{cases} J_{0}(m_{2}) J_{1}(m_{1}) \exp \left[j \left(\omega_{c1} t + \omega t\right)\right] + \\ J_{1}(m_{2}) J_{0}(m_{1}) \exp \left[j \left(\omega_{c1} t + \omega' t\right)\right] \end{cases}$$
(23)
...
| Case | Rnon-blur (m) | R <sub>IF</sub> (m) | $R_{max}(\mathbf{m})$ |
|------|---------------|---------------------|-----------------------|
| 1    | 10.24         | 5.12                | 4.86                  |
| 2    | 15.36         | 10.24               | 9.98                  |
| 3    | 15.36         | 15.36               | 15.1                  |
