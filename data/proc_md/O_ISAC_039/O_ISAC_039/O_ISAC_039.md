...
Xiaojiang Du is with the Department of Electrical and Computer Engineering, Stevens Institute of Technology, Hoboken, NJ 07030 USA.
 E-mail: dxj@ieee.org.
...
Usually, it is assumed that the LEDs follow a Lambertian emission pattern [29], and the *n*th element of the LOS CIR vector can be expressed as

$$[\mathbf{h}_{\text{LOS}}^{(t)}]_n = \frac{(m+1)A_{\text{PD}}\cos^m(\varphi^{(t)})\cos(\psi^{(t)})gT_{\text{s}}}{2\pi d^{(t)^2}}$$

$$\operatorname{rect}\left(\frac{\psi^{(t)}}{\psi_{\text{EOV}}}\right)\delta\left(n\tau_{\text{s}} - \frac{d^{(t)}}{c}\right), \tag{2}$$
...
Since most of the power of the NLOS link is concentrated on the first reflected light [29], the NLOS link is modeled as a diffusion channel of the first reflection, where the wall can be segmented into many small surfaces regarded as reflection elements. Thus, the *n*th element of the NLOS CIR vector is given by
...
where **F** represents the  $N \times N$  discrete Fourier transform (DFT) matrix.  $\tilde{\mathbf{x}}^{(t)} = [0, \tilde{x}_2^{(t)}, \dots, \tilde{x}_{N/2}^{(t)}, 0, \tilde{x}_{N/2}^{(t)}, \dots, \tilde{x}_2^{(t)}]$  is the frequency-domain OFDM data block composed of the pilot and data subcarriers, because of the Hermitian symmetry. As illustrated in Fig. 1b, for the tth LED,  $N_{\rm D}$ pilot subcarriers are randomly distributed with the based algorithms and the emerging sparsity-aware deep-Authorized licensed use limited to: T.C. Cumhurbaskanligi Kutuphanesi. Downloaded on December 14,2025 at 17:43:50 UTC from IEEE Xplore. Restrictions apply.
...
$$\tilde{\mathbf{y}} = \alpha R_{\mathrm{p}} \sum_{t=1}^{N_{\mathrm{t}}} \operatorname{diag}(\tilde{\mathbf{x}}^{(t)}) \mathbf{F}_{L} \mathbf{h}^{(t)} + \tilde{\mathbf{w}}, \tag{6}$$
...
its 
$$n\text{th}$$
 element given by  $[\mathbf{u}^{(t)}]_n = \tilde{y}_{p_n^{(t)}}/(\alpha R_{\text{p}}\tilde{x}_{p_n^{(t)}}^{(t)}), n=1,\ldots,$ 
...
$$\mathbf{F}_{\Lambda} = \begin{bmatrix} \mathbf{F}_{\mathbf{p}}^{(1)} & \cdots & \mathbf{0} \\ \vdots & \ddots & \vdots \\ \mathbf{0} & \cdots & \mathbf{F}_{\mathbf{p}}^{(N_{\mathbf{t}})} \end{bmatrix}_{N,N_{\mathbf{x}}\times N,L}$$
(9)
...
learning-based methods. Moreover, according to (1), (2), and (3), the channel measurement data in  ${\bf u}$  contains plenty of position-related information, so it can also be utilized to estimate the location coordinate  ${\bf c}$  of the receiver. To this end, an MTL-based architecture is proposed to extract the shared features beneficial for both channel estimation and positioning, which is elaborated in detail in Section 3.
...
$$\hat{h}_n = \frac{\left[\mathbf{r}_{\mathrm{C},l}\right]_n}{\left(1 + \frac{\sigma_{\mathrm{C},l}^2}{\sigma_{\mathrm{h}}^2}\right) \left(1 + \frac{N_{\mathrm{t}}L - K}{K} \frac{\mathcal{N}(\left[\mathbf{r}_{\mathrm{C},l}\right]_n;0,\sigma_{\mathrm{C},l}^2)}{\mathcal{N}(\left[\mathbf{r}_{\mathrm{C},l}\right]_n;0,\sigma_{\mathrm{C},l}^2 + \sigma_{\mathrm{h}}^2)}\right)}.$$
(16)
...
$$= \frac{\left[D_{\mathrm{BG}}\left(\mathbf{r}_{\mathrm{C},l};\sigma_{\mathrm{C},l,\mathrm{C},l}\right)\right]_{n}}{\left(1 + \frac{\sigma_{\mathrm{C},l^{2}}}{\theta_{\mathrm{C},l,1}}\right)\left(1 + \sqrt{1 + \frac{\theta_{\mathrm{C},l,1}}{\sigma_{\mathrm{C},l^{2}}}}\exp\left(\theta_{\mathrm{C},l,2} - \frac{\left[\mathbf{r}_{\mathrm{C},l}\right]_{n}^{2}}{2\left(\sigma_{\mathrm{C},l^{2}} + \sigma_{\mathrm{C},l^{4}}/\theta_{\mathrm{C},l,1}\right)}\right)\right)}$$
(17)
...
- 2: repeat
- 3: Înitialize learnable parameters of *i*th layer:  $\mathbf{B}_{S,i} \leftarrow \mathbf{F}_{\Lambda}^T$ ,  $\theta_{S,i} \leftarrow 1$
...
Trained parameters  $\Theta$ , including  $\Theta_{\rm S} = \{\mathbf{B}_{{\rm S},i}, \theta_{{\rm S},i}\}_{i=0}^{N_I-1}$ ,  $\Theta_{\rm C} = \{\mathbf{B}_{{\rm C},l},{_{\rm C},l}\}_{l=0}^{N_L-1}$ , and  $\Theta_{\rm P}$ 
...
Considering the outstanding performance of long short- compromising between the two loss functions of channel term memory (LSTM) in learning the long-term dependency estimation  $\mathcal{L}_{\text{CE}}(\Theta, \Omega)$  and positioning  $\mathcal{L}_{\text{PE}}(\Theta, \Omega)$ . Authorized licensed use limited to: T.C. Cumhurbaskanligi Kutuphanesi. Downloaded on December 14,2025 at 17:43:50 UTC from IEEE Xplore. Restrictions apply.
...
$$\mathbb{E}\left[\left\|\hat{\mathbf{h}}^{(t)} - \mathbf{h}^{(t)}\right\|_{2}^{2}\right] \ge \frac{L}{N_{\mathrm{p}}} \sigma_{\mathrm{w}}^{2}.$$
 (22)
...
$$p_{\mathbf{u}^{(t)}|\mathbf{h}^{(t)}}\left(\mathbf{u}^{(t)};\mathbf{h}^{(t)}\right) = \frac{1}{\left(2\pi\sigma_{\mathbf{w}}^{2}\right)^{N_{\mathbf{p}}/2}} \exp\left\{-\frac{1}{2\sigma_{\mathbf{w}}^{2}} \left\|\mathbf{u}^{(t)} - \mathbf{F}_{\mathbf{p}}^{(t)}\mathbf{h}^{(t)}\right\|_{2}^{2}\right\}. \quad (23)$$

Then, the Fisher information matrix (FIM) [34] for channel estimation in (7) can be obtained by the conditional PDF in (23) as

$$[\mathbf{J}_{\mathbf{h}}]_{a,b} \triangleq -\mathbb{E}\left[\frac{\partial^{2} \ln\left(p_{\mathbf{u}^{(t)}\mid\mathbf{h}^{(t)}}\left(\mathbf{u}^{(t)};\mathbf{h}^{(t)}\right)\right)}{\partial h_{a}^{(t)}\partial h_{b}^{(t)}}\right]$$

$$= \frac{1}{\sigma_{\mathbf{w}}^{2}}\left[\left(\mathbf{F}_{\mathbf{p}}^{(t)}\right)^{H}\mathbf{F}_{\mathbf{p}}^{(t)}\right]_{a,b},$$
(24)
...
$$= \sigma_{w}^{2} \operatorname{tr}\left(\left(\left(\mathbf{F}_{p}^{(t)}\right)^{H} \mathbf{F}_{p}^{(t)}\right)^{-1}\right). \tag{25}$$
...
where  $\lambda_1, \lambda_2, \dots, \lambda_L$  denote the eigenvalues of the matrix  $(\mathbf{F}_{\mathrm{p}}^{(t)})^H \mathbf{F}_{\mathrm{p}}^{(t)}$ . It is worth noting that, the inequation in (26) reaches equality if and only if the condition  $\lambda_1 = \lambda_2 = \dots = \lambda_L$  holds, i.e., the columns in the partial DFT matrix
...
**Remark 1.** The CRLB in Corollary 1 can be achieved only if the inequation in (26) reaches equality, i.e., the  $N_{\rm p} \times L$  partial DFT matrix  ${\bf F}_{\rm p}^{(t)}$  has orthogonal columns. Fortunately, since the  ${\bf F}_{\rm p}^{(t)}$  contains L rows and  $N_{\rm p}$  columns of the DFT matrix  ${\bf F}$ , which is a perfectly orthogonal matrix, and the pilot subcarriers are chosen with a random pattern,  ${\bf F}_{\rm p}^{(t)}$  has approximate orthogonality. Thus, the CRLB in (22) can be asymptotically approached.
...
$$\geq \frac{1}{N_{\text{D}}} \left(\frac{2\pi\sigma_{\text{w}}}{(m+1)(m+3)A_{\text{PD}}gT_{\text{s}}z^{m+1}}\right)^{2} \sum_{t=1}^{N_{\text{t}}} \left(d^{(t)}\right)^{2m+8}.$$
 (28)
...
of  $\mathcal{N}(\mathbf{0}, \sigma_{\Gamma}^2 \mathbf{I}_{N_{\mathrm{t}}})$ . The PDF of  $\mathbf{h}_{\mathrm{w}}$  conditioned by the distance  $\mathbf{d} = [d^{(1)}, d^{(2)}, \dots, d^{(N_{\mathrm{t}})}]$  can be expressed as
...
$$\mathbf{A} = \begin{pmatrix} \frac{\partial h_{\Gamma,1}}{\partial d^{(1)}} & \cdots & \frac{\partial h_{\Gamma,N_t}}{\partial d^{(1)}} \\ \vdots & \ddots & \vdots \\ \frac{\partial h_{\Gamma,1}}{\partial d^{(N_t)}} & \cdots & \frac{\partial h_{\Gamma,N_t}}{\partial d^{(N_t)}} \end{pmatrix}$$

$$= -\frac{(m+1)(m+3)A_{PD}gT_sz^{m+1}}{2\pi}$$

$$\begin{pmatrix} (d^{(1)})^{m+4} & \cdots & 0 \\ \vdots & \ddots & \vdots \\ 0 & \cdots & (d^{(N_t)})^{m+4} \end{pmatrix}^{-1}, \quad (34)$$
...
$$[\mathbf{J}_{\mathbf{d}}]_{t,t} = \left(\frac{(m+1)(m+3)A_{\mathrm{PD}}gT_{\mathbf{s}}z^{m+1}}{2\pi\sigma_{\Gamma}(d^{(t)})^{m+4}}\right)^{2}.$$
 (36)
...
$$\sigma_{\Gamma}^2 \ge \frac{1}{L} \cdot \frac{L}{N_{\rm D}} \sigma_{\rm w}^2 = \frac{\sigma_{\rm w}^2}{N_{\rm D}}.$$
 (38)
...
$$\bar{\boldsymbol{\Theta}}_{\tau,s} = \bar{\boldsymbol{\Theta}}_{\tau,s-1} - \frac{1}{N_{\rm r}} \sum_{r=1}^{N_{\rm r}} \mathbf{G}_{\tau,s}^{(r)}.$$
 (40)
...
**Assumption 2.** The variance of the gradient of the loss function is bounded by  $\sigma_{g'}^2$ , i.e.,  $\mathbb{E}_{\mathbf{\Omega}_r^{\tau,s} \sim \mathbf{\Omega}_r}[\|\nabla \mathcal{L}(\mathbf{\Theta}, \mathbf{\Omega}_r^{\tau,s}) - \nabla \mathcal{L}_r(\mathbf{\Theta})\|_2^2] \le \sigma_{g'}^2, \forall \mathbf{\Theta}, \forall r \in \{1, 2, \dots, N_r\}.$ 
...
Subsequently, we bound the deviation of the local model weights  $\Theta_{\tau,s}^{(r)}$  from the average weights  $\bar{\Theta}_{\tau,s}$  using Lemma 1 as follows. Then, we analyze the convergence rate of the proposed MTFL framework in Corollary 3 using the average of the expected squared gradient norm, which is widely adopted to characterize the convergence rate [37].
...
**Proof.** Consider a certain communication round  $\tau \geq 1$  and local step  $s \geq 1$ . Based on (20) and the initialization of  $\mathbf{\Theta}_{\tau,0}^{(r)}$  in Line 4 of Algorithm 2, for  $r \in \{1,2,\ldots,N_{\mathrm{r}}\}$ , we have
...
Similarly, based on (40) and the initialization of  $\Theta_{\tau,0}^{(r)}$  in Algorithm 2, we have
...
\leq 2\zeta^{2} \mathbb{E}\left[\left\|\sum_{\xi=1}^{s} \frac{1}{N_{r}} \sum_{r=1}^{N_{r}} \mathbf{G}_{\tau,\xi}^{(r)}\right\|_{2}^{2} + \left\|\sum_{\xi=1}^{s} \mathbf{G}_{\tau,\xi}^{(r)}\right\|_{2}^{2}\right] \\
...
where the first three inequalities are derived from the Jensen's inequality  $\|\sum_{i=1}^n \mathbf{z}_i\|_2^2 \leq \sum_{i=1}^n \frac{1}{n} \|\mathbf{z}_i\|_2^2$ , and the last inequality holds based on Assumption 3.

**Corollary 3.** With Assumptions 1, 2, and 3, if the learning rate is  $\zeta = \sqrt{\frac{N_{\rm r}}{N_{\rm s}T}} \leq \frac{1}{C}$ , the average of the expected squared gradient norm is bounded by
...
$$\mathbb{E}\left[\mathcal{L}(\bar{\boldsymbol{\Theta}}_{\tau,s})\right] \leq \mathbb{E}\left[\mathcal{L}(\bar{\boldsymbol{\Theta}}_{\tau,s-1})\right] + \underbrace{\frac{C}{2}}_{T_2} \underbrace{\mathbb{E}\left[\left\|\bar{\boldsymbol{\Theta}}_{\tau,s} - \bar{\boldsymbol{\Theta}}_{\tau,s-1}\right\|_{2}^{2}\right]}_{T_1} + \underbrace{\mathbb{E}\left[\left\langle\nabla\mathcal{L}(\bar{\boldsymbol{\Theta}}_{\tau,s-1}), \bar{\boldsymbol{\Theta}}_{\tau,s} - \bar{\boldsymbol{\Theta}}_{\tau,s-1}\right\rangle\right]}_{T_2}.$$
 (46)
...
where the second equality is derived by the law of iterated expectation, since  $\Theta_{1,2}^{(r)}$  is dependent on the selection
...
$$\mathbb{E}\left[\mathcal{L}(\bar{\boldsymbol{\Theta}}_{\tau,s})\right] \\
\leq \mathbb{E}\left[\mathcal{L}(\bar{\boldsymbol{\Theta}}_{\tau,s-1})\right] - \frac{\zeta}{2}\mathbb{E}\left[\left\|\nabla\mathcal{L}(\bar{\boldsymbol{\Theta}}_{\tau,s-1})\right\|_{2}^{2}\right] \\
...
\leq 4\zeta^{2} N_{s}^{2} G^{2} C^{2}, \tag{51}$$

where the first inequality holds based on the Jensen's inequality  $\|\sum_{i=1}^n \frac{1}{n} \mathbf{z}_i\|_2^2 \le \sum_{i=1}^n \frac{1}{n} \|\mathbf{z}_i\|_2^2$ , the second inequality comes from the smoothness in Assumption 1, and the last inequality is obtained from Lemma 1.

Substituting (51) to (50) and rearrange the terms, we have

$$\frac{\zeta}{2} \mathbb{E} \left[ \left\| \nabla \mathcal{L} (\bar{\mathbf{\Theta}}_{\tau,s-1}) \right\|_{2}^{2} \right] \\
\leq -\frac{\zeta - \zeta^{2} C}{2} \mathbb{E} \left[ \left\| \frac{1}{N_{r}} \sum_{r=1}^{N_{r}} \nabla \mathcal{L}_{r} \left( \mathbf{\Theta}_{\tau,s-1}^{(r)} \right) \right\|_{2}^{2} \right] + \frac{C}{2N_{r}} \zeta^{2} \sigma_{g}^{2} \\
+ \mathbb{E} \left[ \mathcal{L} (\bar{\mathbf{\Theta}}_{\tau,s-1}) \right] - \mathbb{E} \left[ \mathcal{L} (\bar{\mathbf{\Theta}}_{\tau,s}) \right] + 2\zeta^{3} N_{s}^{2} G^{2} C^{2} \\
\leq \mathbb{E} \left[ \mathcal{L} (\bar{\mathbf{\Theta}}_{\tau,s-1}) \right] - \mathbb{E} \left[ \mathcal{L} (\bar{\mathbf{\Theta}}_{\tau,s}) \right] + 2\zeta^{3} N_{s}^{2} G^{2} C^{2} + \frac{C}{2N_{r}} \zeta^{2} \sigma_{g}^{2}, \tag{53}$$

where the second inequality holds since  $0 < \zeta \le \frac{1}{C}$ . Subsequently, summing (52) over  $s \in \{1, 2, ..., N_s\}$  and  $\tau \in \{1, 2, ..., T\}$  yields
...
| Electro-optical conversion efficiency | α                  | 1  W/A             |
| Average reflectance of walls          | ar ho              | 0.7                |
| FOV angle of PDs                      | $\psi_{	ext{FOV}}$ | $90^{\circ}$       |
| PD effective area                     | $A_{ m PD}$        | $1~{ m cm}^2$      |
| Optical filter gain                   | $T_{ m s}^{D}$     | 1                  |
| Optical concentrator gain             | g                  | 1                  |
| PD responsivity                       | $ar{R}_{ m p}$     | $0.6~\mathrm{A/W}$ |

$$\frac{\zeta}{2} \sum_{\tau=1}^{T} \sum_{s=1}^{N_{s}} \mathbb{E} \left[ \left\| \nabla \mathcal{L} (\bar{\mathbf{\Theta}}_{\tau,s-1}) \right\|_{2}^{2} \right] 
\leq \mathbb{E} \left[ \mathcal{L} (\bar{\mathbf{\Theta}}_{1,0}) \right] - \mathbb{E} \left[ \mathcal{L} (\bar{\mathbf{\Theta}}_{T,N_{s}}) \right] + 2\zeta^{3} G^{2} C^{2} N_{s}^{3} T 
+ \frac{\zeta^{2} \sigma_{g}^{2} C N_{s} T}{2 N_{r}} 
\leq \mathcal{L} (\bar{\mathbf{\Theta}}_{0}) - \mathcal{L} (\mathbf{\Theta}^{*}) + 2\zeta^{3} G^{2} C^{2} N_{s}^{3} T + \frac{\zeta^{2} \sigma_{g}^{2} C N_{s} T}{2 N_{r}}, \quad (53)$$
...
$$\leq 2 \frac{\mathcal{L}(\bar{\boldsymbol{\Theta}}_{0}) - \mathcal{L}(\boldsymbol{\Theta}^{*})}{\zeta N_{s} T} + 4\zeta^{2} G^{2} C^{2} N_{s}^{2} + \frac{\zeta C \sigma_{g}^{2}}{N_{s}}. \tag{54}$$

By setting the learning rate as  $\zeta = \sqrt{\frac{N_{\rm r}}{N_{\rm s}T'}}$  we have (45).  $\Box$ 
...
#### Performance of the MTFL Framework for VIPAC
...
Fig. 9. CDF with respect to the positioning error for the proposed MTLbased network and the benchmark schemes
...
Fig. 11. Training loss over the local datasets of the MTFL framework for VIPAC with respect to communication rounds in federated learning. Authorized licensed use limited to: T.C. Cumhurbaskanligi Kutuphanesi. Downloaded on December 14,2025 at 17:43:50 UTC from IEEE Xplore. Restrictions apply.
