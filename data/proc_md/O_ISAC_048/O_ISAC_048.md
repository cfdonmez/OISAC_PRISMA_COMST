...
where  $X^+$  (k,m) and  $H_c$  (k) are the discrete Fourier transform (DFT) of  $x^+$   $(n/R_s)$  and  $h_c$   $(n/R_s)$ , respectively. When X (k,m) obeys the Gaussian distribution independently, the performance metric of the communication subsystem, i.e., the spectral efficiency, can be expressed as
...
$$\sigma_{w_p}^2 = \sigma_x^2 \left( \lambda_b^2 \left( 1 - Q \left( \lambda_b \right) \right) + \lambda_b \phi \left( \lambda_b \right) + Q \left( \lambda_b \right) - K^2 - \left( \lambda_b \left( 1 - Q \left( \lambda_b \right) \right) - \phi \left( \lambda_b \right) \right)^2 \right), \tag{13}$$
...
(P1-1): 
$$\max_{k} C\left(b, \tilde{P}\left(k\right)\right), \tag{16}$$
...
the clipping noise is independent of the normalized power allocation, which does not affect the solution of (P1-1). On the other hand, although the DC bias b affects K,  $\sigma_{w_p}^2$ , and  $P-b^2$  simultaneously,  $\gamma_c(b)$  is generally dominated by the numerator in (14a) for large  $N_c$ , i.e.,

$$\gamma_c\left(b\right) \approx \frac{2K^2\left(P - b^2\right)}{N_c \Delta f}, \quad \text{if} \quad \frac{N_c \Delta f}{2} \gg P_{w_p}\left(k\right).$$
 (17)
...
Once  $b^*$  is obtained by solving (P1-1), the normalized power allocation  $\tilde{P}(k)$  for each subcarrier is further optimized to maximize the spectral efficiency. Therefore, the sub-problem to optimize power allocation for each subcarrier is formulated as

(P1-2): 
$$\max_{\tilde{P}(k)} C\left(b^*, \tilde{P}(k)\right), \tag{18}$$

s.t. 
$$I\left(b^{*}, \tilde{P}\left(k\right)\right) \geq \varsigma^{2},$$
 (18a)
...
$$\xi_0(\mu, \eta, k) = \max \left\{ \mu - \eta \gamma_s(b^*) k^2, \frac{1}{\left(P_m + 1/\left(\gamma_c(b^*) |H_c(k)|^2\right)\right)} \right\}, \quad (19a)$$

$$\tilde{P}_{c}\left(k\right) = \left\{\frac{1}{\xi_{0}\left(\mu^{*}, 0, k\right)} - \frac{1}{\gamma_{c}\left(b^{*}\right)\left|H_{c}\left(k\right)\right|^{2}}\right\}^{+}, \ \ (19b)$$
...
1: Initialize 
$$\eta^{(0)} \leftarrow 0$$
,  $\mu^{(0)} \leftarrow 0$ ,  $i \leftarrow 0$ .  
2: **while**  $|\mu^{(i+1)} - \mu^{(i)}| \ge \varepsilon_{\mu}$  or  $|\eta^{(i+1)} - \eta^{(i)}| \ge \varepsilon_{\eta}$  **do**

3: Calculate upper bound  $\mu_c(i)$  by (27a).

4: Solve (26a) on  $\left[\mu^{(i)}, \mu_c(i)\right]$  to obtain  $\mu^{(i+1)}$ .

5: Calculate upper bound  $\eta_c(i)$  by (27b).

6: Solve (26b) on  $[\eta^{(i)}, \eta_c(i)]$  to obtain  $\eta^{(i+1)}$ .
...
Then, by defining  $l_m = \lfloor N/2 - 1/\left(2P_m\right) \rfloor$ , the normalized power allocation for the  $k_l$ -th subcarrier is expressed as
...
3) Case C: In this case, the sensing constraint cannot be satisfied by  $\tilde{P}_c(k)$ , while the problem is still feasible for  $\tilde{P}_s(k)$ . Consequently, a trade-off exists between communication and sensing metrics, and the optimal power allocation yielded by the KKT conditions is expressed as
...
$$\mathcal{R}: \ \gamma_{s}(b^{*}) \eta \leq \mu \leq \gamma_{s}(b^{*}) l_{1}(\eta)^{2} \eta + \gamma_{c}(b^{*}) \left| H_{c}(l_{1}(\eta)) \right|^{2}, \tag{29}$$
