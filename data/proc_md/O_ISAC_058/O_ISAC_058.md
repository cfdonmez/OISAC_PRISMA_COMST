...
On the UE, we amplify the signal received by the HA using a low noise amplifier (LNA). Then, we use the ED to downconvert the MMW signal to the IF band. This scheme aims to reduce the device pressure on the UE while also eliminating the frequency offset caused by heterodyning two free-run lasers. The electrical signal after the ED can be represented as
