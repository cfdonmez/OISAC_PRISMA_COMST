...
Yunfeng Wen is with the Department of Electronic Engineering, Tsinghua University, Beijing 100084, China (e-mail: wenyf22@mails.tsinghua.edu.cn). Fang Yang is with the Department of Electronic Engineering, Tsinghua University, Beijing 100084, China, and also with the Research Institute of Tsinghua University in Shenzhen, Tsinghua University, Shenzhen 518057,

China (e-mail: fangyang@tsinghua.edu.cn). Jian Song is with the Department of Electronic Engineering, Tsinghua University, Beijing 100084, China, also with the Shenzhen International Graduate School, Tsinghua University, Shenzhen 518055, China, and also with the Research Institute of Tsinghua University in Shenzhen, Tsinghua University, Shenzhen 518057, China (e-mail: jsong@tsinghua.edu.cn).
...
$$x^{+}(n) = \mathcal{K}x(n) + w_{n}(n), \ X^{+}(k) = \mathcal{K}X(k) + W_{n}(k)$$
 (3)
...
$$R_{w_p}(n-m) = \mathbb{E}\big(g_p(x(n))g_p(x(m))\big) \tag{6}$$
...
$$\mathcal{I}(r) = \int_{-\sigma_x^2}^r \int_{-\sigma_x^2}^s \left[ \frac{1}{2\pi\sqrt{\sigma_x^4 - t^2}} \exp\left(-\frac{b^2}{\sigma_x^2 + t}\right) \right] dt ds \quad (8)$$
...
$$p(L_t; D) = \frac{1}{L_t \sqrt{2\pi\sigma_t^2}} \exp\left(-\frac{1}{2\sigma_t^2} \left(\ln\frac{L_t}{\bar{L}_t} + \frac{\sigma_t^2}{2}\right)^2\right)$$
(14)
...
where  $G_s$  and  $\mathfrak{R}$  denote the interface term for sensing and target reflectivity, respectively.

<span id="page-3-2"></span><sup>1</sup>We assume that the reflected optical beam has the same divergence  $\theta$  angle as the transmitted one, which is achievable by a mirror reflector.
...
$$I = \int_{0}^{+\infty} p(L_{t}; D) \frac{8\pi^{2}}{NT^{2}} \sum_{k=1}^{N/2-1} k^{2} \gamma_{s}(k) dL_{t}$$

$$:= \int_{0}^{+\infty} p(L_{t}; D) \tilde{I}(L_{t}) dL_{t}$$
(26)
...
4: if  $\tilde{C}(b_l^-, \tilde{P}(k); \tilde{L}_t) \ge \tilde{C}(b_r^-, \tilde{P}(k); \tilde{L}_t)$  then
...
$$\xi_0(\mu, \eta, k) = \max \left\{ \mu - \eta \tilde{\gamma}_s(k) k^2, 1 / (\tilde{P}_m + 1 / \tilde{\gamma}_c(k)) \right\} (36a)$$

$$\tilde{P}_c(k) = \left\{ \frac{1}{\xi_0(\mu_c^*, 0, k)} - \frac{1}{\tilde{\gamma}_c(k)} \right\}^+$$
(36b)

where the optimal dual variable  $\mu_c^*$  can be obtained by solving (32e). If the sensing constraint is still satisfied, i.e.,

$$\sum_{k=1}^{N/2-1} k^2 \tilde{\gamma}_s(k) \tilde{P}_c(k) \ge \frac{NT^2 \zeta_0^2}{8\pi^2} := \tilde{\zeta}_0^2$$
 (37)
...
$$\sum_{k=1}^{N/2-1} k^2 \tilde{\gamma}_s(k) \tilde{P}_s(k) < \tilde{\zeta}_0^2$$
 (40)
...
$$\xi_2(\mu_c^*, \eta_c^*) = \tilde{\varsigma}_0^2 \tag{42b}$$
...
$$S_1: \xi_1(\mu, \eta) = \frac{1}{2}$$
 (44a)

$$S_2$$
:  $\xi_2(\mu, \eta) = \tilde{\zeta}_0^2$ . (44b)
...
*Proof:* As illustrated in Fig. 3, Lemma 1 indicates the existence of  $(\mu_c^*, \eta_c^*)$ , and Lemma 2 guarantees that  $(\mu^{(i)}, \eta^{(i)})$  will not leave the feasible region during the iteration. Furthermore, we have the initial points  $\mu^{(0)}$  <  $\mu_c^*$ ,  $\eta^{(0)} < \eta_c^*$  since  $\mu_c^* > 0$ ,  $\eta_c^* > 0$ . Supposing that  $\eta^{(j)} < \eta_c^*$ , solving  $\xi_1(\mu^{(j+1)}, \eta^{(j)}) = 1/2$  yields  $\mu^{(j)} < \mu^{(j+1)} < \mu_c^*$ . Similarly, assuming that  $\mu^{(j+1)} < \mu_c^*$ , solving  $\xi_2(\mu^{(j+1)}, \eta^{(j+1)}) = \tilde{\zeta}_0^2$  yields  $\eta^{(j)} < \eta^{(j+1)} < \eta_c^*$ . Therefore, the distance between the current solution and the optimal solution declines as the algorithm proceeds, i.e.,

$$|\mu^{(j+1)} - \mu_c^*| < |\mu^{(j)} - \mu_c^*| \tag{47a}$$

<span id="page-7-11"></span>
$$|\eta^{(j+1)} - \eta_c^{c}| < |\eta^{(j)} - \eta_c^{c}|. \tag{47b}$$
...
(P2): 
$$\max_{\substack{b \ \tilde{P}(k)}} \tilde{I}(b, \tilde{P}(k); \tilde{L}_t)$$
 (48a)
...
$$\sum_{k=1}^{N/2-1} \log(1 + \tilde{\gamma}_c(k)\tilde{P}_s(k)) \ge C_0 B T_o := \tilde{C}_0$$
 (51)
...
| Duration of an OFDM symbol      | $T_o$              | 3.25, 4.5, 7 μs                      |  |
| Total power                     | $P_t$              | 1 W                                  |  |
...
| Receiver aperture               | A                  | $10~{\rm cm}^2$                      |  |
| Target reflectivity             | 93                 | 0.5                                  |  |
...
<span id="page-9-4"></span><span id="page-9-3"></span>TABLE II
OPTIMAL DC BIAS W.R.T. DIFFERENT SENSING PRECISIONS AND
...
$$\mathcal{O}\left(N\left(\log(1/\varepsilon_b) + 2\left(\log(1/\varepsilon_\mu\right) + \log(1/\varepsilon_\eta)\right)^2\right)\right).$$
 (57)
...
$$= \frac{1}{2\pi\sqrt{\sigma_x^4 - r^2}} \exp\left(-\frac{b^2}{\sigma_x^2 + r}\right) \quad (60)$$
...
$$R_{w_p}(n) = \mathbb{E}\left(\left(w_p(n)\right)^2\right) = \mathcal{I}\left(\sigma_x^2\right) + C_1\sigma_x^2 + C_2.$$
 (63)
...
$$I = \left(W^{\mathcal{H}} \frac{\partial s(\tau_0)}{\partial \tau_0}\right)^{\mathcal{H}} \operatorname{diag}(S_{\mathbf{w}})^{-1} \left(W^{\mathcal{H}} \frac{\partial s(\tau_0)}{\partial \tau_0}\right)$$
$$:= S'(\tau_0)^{\mathcal{H}} \operatorname{diag}(S_{\mathbf{w}})^{-1} S'(\tau_0). \tag{67}$$

For the sensing subsystem, the *n*th elements of  $s(\tau_0)$  and w are defined, respectively, as

$$s(n; \tau_0) = H_s \mathcal{K} x(n - \tau_0 R_s) \tag{68a}$$
...
$$I = \sum_{k=0}^{N-1} \frac{|S'(k; \tau_0)|^2}{S_{\mathbf{w}}(k)} = \frac{8\pi^2}{NT^2} \sum_{k=0}^{N/2-1} k^2 \gamma_{\mathcal{S}}(k).$$
 (70)

## <span id="page-13-5"></span>APPENDIX C PROOF OF LEMMA 1

As indicated by (43a) and (43b),  $\xi_1(\mu, \eta)$  and  $\xi_2(\mu, \eta)$  are both continuous, nonincreasing w.r.t.  $\mu$ , and nondecreasing w.r.t.  $\eta$  in the first quadrant. In addition, the existence of  $\mathcal{S}_1$  and  $\mathcal{S}_2$  is guaranteed by
...
$$\xi_2(\zeta_1(\eta^+), 0) \ge \tilde{\zeta}_0^2$$
 (73)
