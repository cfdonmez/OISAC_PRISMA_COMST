...
1557-9611 © 2022 IEEE. Personal use is permitted, but republication/redistribution requires IEEE permission. See https://www.ieee.org/publications/rights/index.html for more information.
...
| Positioning algorithm     | RSS        | RSS           | PDOA      | RSS      | RSS          |
|                           | trilatera- | trilater-     | trilater- | finger-  | trilater-    |
|                           | tion       | ation         | ation     | printing | ation        |
| Lighting constraints      | No         | No            | No        | No       | Yes          |
| Realistic<br>LED<br>model | No         | No            | No        | No       | Yes          |
| Data rate                 | ND         | 1.31<br>Gbps  | ND        | ND       | 32<br>Mbps   |
| Positioning               | 2.9 to     | 4.3 to 8.1 cm | 8.1 cm    | 4.5 to   | 7.17         |
| accuracy                  | 21.2 cm    | 20 cm         | 0.1 Cm    | 10.7 cm  | cm           |
...
$$N_{ss} = \left| \frac{f_{samp}}{B_{sc}} \right| = \left| \frac{2B}{B_{sc}} \right|,\tag{8}$$
...
$$\hat{I}_{i}(t) = \sum_{n=1}^{m} \left[ X_{I}^{i,n}(t) \otimes f_{I}^{i,n}(t) - X_{Q}^{i,n}(t) \otimes f_{Q}^{i,n}(t) \right]. \tag{12}$$
...
$$X_I^{'i,n} = r_1^{i,n} \otimes f_I^{'i,n},$$
 (15)
...
$$\hat{\Phi}_{rx,i}^r = \frac{\sqrt{P_{rx,i}}}{R_{\text{PD}}} = \frac{\sqrt{\mathbb{V}(I_{rx,i}(t))}}{R_{\text{PD}}}.$$
 (18)
...
UE using the values  $\hat{\Phi}_{tx,i}^r$  and  $\hat{\Phi}_{rx,i}^r$ . According to the optical wireless channel model, we know that [35]:

<span id="page-6-1"></span>
$$\frac{\Phi_{rx,i}^{r}}{\Phi_{tx,i}^{r}} = \frac{A_{\text{PD}}(m_{i}+1)}{2\pi d_{i}^{2}} \cos^{m_{i}}(\phi_{i}) \cos(\psi_{i}) T_{f}(\psi_{i}) G_{c}(\psi_{i}). \quad (19)$$
...
|-----------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--|--|--|--|--|
| Parameter                                     | Value                                                                                                                                                                                             |  |  |  |  |  |
| 3D coordinates                                | $\begin{array}{l} \text{AP}_{1,2,3,4}\colon (\pm 1.5,\pm 1.5,2.5) \\ \text{AP}_{5,6}\colon (\pm 1.5,0,2.5) \\ \text{AP}_{7,8}\colon (0,\pm 1.5,2.5) \\ \text{AP}_{9}\colon (0,0,2.5) \end{array}$ |  |  |  |  |  |
...
|                                     | $16 (R_b = 16 \text{ Mbps})$<br>$32 (R_b = 20 \text{ Mbps})$ |                     |  |
| QAM order (M)                       | 64 ( $R_b = 24 \text{ Mbps}$ )                               |                     |  |
|                                     | 128 ( $R_b = 28 \text{ Mbps}$ )                              |                     |  |
|                                     | 256 ( $R_b = 32 \text{ Mbps}$ )                              |                     |  |
...
These results show that despite lighting and LED constraints, the proposed *m*-CAP and RSS-based joint communication and sensing system could be performing enough to <span id="page-11-39"></span>support part of the current LBS services such as 4K broadcasting, which currently requires between 15 and 18 Mbps data rates [\[41\]](#page-12-1), or some virtual reality applications [\[42\]](#page-12-2), but also future services that will come with beyond 5G and 6G technologies.
