...
|------------------------|---------------------------------------|-----------------------------------------|-----------------------------------------|----------------------------------------|----------------------------------------|
|                        | Proximity [23], [24]                  | <b>√</b>                                |                                         |                                        |                                        |
| VLP                    | Fingerprinting [25], [26], [52], [53] | <b>√</b>                                |                                         |                                        |                                        |
|                        | Camera [27], [28]                     |                                         | <b>√</b>                                |                                        |                                        |
|                        | ToA [29]                              |                                         |                                         |                                        | LoS                                    |
|                        | AoA [30]–[32]                         |                                         |                                         |                                        | LoS                                    |
|                        | RSS [33]–[37]                         |                                         |                                         |                                        | LoS                                    |
|                        | OOK/PPM                               |                                         |                                         |                                        | LoS                                    |
| VLC                    | DCO-OFDM [42]                         |                                         |                                         |                                        | TD/FD                                  |
| VLC                    | ACO-OFDM[43], [44]                    | <b>√</b>                                |                                         | <b>√</b>                               | TD/FD                                  |
|                        | LACO-OFDM[45]–[47]                    | <b>√</b>                                |                                         |                                        | TD/FD                                  |
| Proposed VLPC-DCO-OFDM |                                       | <b>√</b>                                | <b>√</b>                                | $\sqrt{}$                              | Radar                                  |
...
<span id="page-3-7"></span><span id="page-3-6"></span>According to the generalized Lambertian pattern, the LoS path spanning from a LED to a PD seen in Fig. 1 is formulated as [41], [62], [63]:
...
| $\mathbf{n}_D = \left[\sin \varphi_r^{\alpha} \cos \varphi_r^{\beta}, \sin \varphi_r^{\alpha} \sin \varphi_r^{\beta}, \cos \varphi_r^{\alpha}\right]$ | Unit normal vector of destina- |  |
...
| Nota-<br>tion                            | Parameter                                | Example Value                       |
|------------------------------------------|------------------------------------------|-------------------------------------|
| $\phi_{1/2}$                             | Half-power semi-angle of LED             | 60°                                 |
| $A_{ m PD}$                              | Detector physical area of PD             | 1 cm <sup>2</sup>                   |
| θ                                        | LED tilt angle                           | 00                                  |
| $\varphi_r^{\alpha}$ $\varphi_r^{\beta}$ | Polar angle of PD                        | arbitrary                           |
| $\varphi_r^{\beta}$                      | Azimuth angle of PD                      | arbitrary                           |
| $g_{ m of}(arphi_r)$                     | Gain of optical filter at LED            | 2.5481                              |
| n                                        | Refractive index of optical concentrator | 1.5                                 |
| $\psi_{	ext{FoV}}$                       | Field-of-view semi-angle of PD           | 60°                                 |
| e                                        | Electron charge                          | $1.602 \times 10^{-19} \text{ C}$   |
| $R_p$                                    | Responsivity of PD                       | 0.54 A/W                            |
| $I_{ m bg}$                              | Background current                       | $\int 5100 \ \mu A$                 |
| $I_2$                                    | Noise bandwidth factor                   | 0.562                               |
| $k_B$                                    | Boltzmann's constant                     | $1.381 \times 10^{-23} \text{ J/K}$ |
| $T_k$                                    | Absolute temperature                     | 300 K                               |
| Γ                                        | Channel noise factor of the FET          | 1.5                                 |
| $G_{ m oc}$                              | Open-loop voltage gain                   | 10                                  |
| η                                        | Field capacitance per unit area          | 11.2pF/cm <sup>2</sup>              |
| $g_m$                                    | FET trans-conductance                    | 30 ms                               |
| $I_3$                                    | Noise bandwidth factor                   | 0.0868                              |
| $\Delta f$                               | Subcarrier spacing                       | 240 kHz                             |
...
$$= \frac{1}{\sqrt{M}} [\eta_0 + (-1)^m] + \frac{2}{\sqrt{M}} \operatorname{Re} \left( \sum_{\overline{m}=1}^{\frac{M}{2}-1} \overline{s}[n,\overline{m}] \omega_M^{m\overline{m}} \right),$$
(5)
...
model of (6) to be free of inter-symbol interference, the CP length has to be higher than the maximum delay, i.e.  $M_{cp} > \max_{\forall p} l_p$ , which is easy to achieve in high-bandwidth VLPC. For example, when a subcarrier spacing (SCS) of 240kHz and 10% CP overhead are used, the NLoS paths spreading up to  $\frac{10\%}{10\times SCS} = 125$ m are covered by CP.
...
which remains real- and positive-valued for IM/DD only when we have  $\omega_M^{(m-l_p)(M-\overline{m})}=\omega_M^{-(m-l_p)\overline{m}}\omega_M^{(m-l_p)M}=\omega_M^{-(m-l_p)\overline{m}}$  for integer delay indices  $l_p=\lfloor\frac{\tau_p}{T}\rfloor$ , leading to:
...
where the ranging noise is given by  $\widetilde{v}[0,l] = \frac{1}{\sqrt{M}} \sum_{\overline{m}=0}^{M-1} \overline{v}[0,\overline{m}] \omega_M^{\overline{m}l}$ . Therefore, (15) has peaks at  $\{l=l_p\}_{p=0}^{P-1}$ , which are expressed by:
...
where OFDM radar obtains  $\hat{l}_0$  based on (17), while the RSS estimates the LED-PD distance based on:
...
Based on (19), the RSS-aided estimate  $\hat{d}'_{SD}$  is adopted only when its corresponding delay index  $\hat{l}'_0 = \lfloor \frac{\hat{d}'_{SD}}{\xi} BW \rfloor$  equals to the Radar-estimated delay index, i.e.  $\hat{l}_0 = \hat{l}'_0$ . This scenario is encountered under two conditions. Firstly, the LoS path is separated from the NLoS paths, as summarized by Property 2. Secondly, the LED/PD parameters including q,  $\mathbf{n}_S$ ,  $\mathbf{n}_D$ ,  $g_{of}(\varphi_r)$  and  $g_{oc}(\varphi_r)$  are fixed or known. Otherwise, we have  $\hat{l}_0 \neq \hat{l}'_0$ , and the radar-based range estimation is used.
...
$$\hat{\overline{h}}_{\overline{m}} = \sum_{l=0}^{\hat{L}} \hat{h}_l \omega_M^{-\overline{m}l}, \tag{23}$$
...
$$\bar{s}'[n,\overline{m}] = \frac{\overline{y}[n,\overline{m}]}{\hat{h}_{\overline{m}}},$$
(26)
...
$$p(\overline{y}[n,\overline{m}]|\overline{s}[n,\overline{m}]) = \frac{1}{\sqrt{2\pi N_0}} \exp\left(-\frac{|\overline{y}[n,\overline{m}] - \overline{h}_{\overline{m}}\overline{s}[n,\overline{m}]|^2}{N_0}\right), \quad (27)$$

which leads to the following Continuous-input Continuous-output Memoryless Channel (CCMC) sum rate [72], [73], [74]:

$$C^{\text{CCMC}}(P_t) = \frac{\text{BW}}{M/2 - 1} E\left[ \sum_{\overline{m}=1}^{\frac{M}{2} - 1} \log_2 \left( 1 + \frac{|\overline{h}_{\overline{m}}|^2}{N_0} \right) \right]. \tag{28}$$
...
where the maximum achievable rate is given by a Gaussian-distributed CSI estimation error with zero mean and a variance of  $\sigma_{CE}^2 = E(|\overline{h}_{\overline{m}} - \overline{h}_{\overline{m}}|^2)$ .
...
<span id="page-8-12"></span><span id="page-8-3"></span>the resultant Discrete-input Continuous-output Memoryless Channel (DCMC) sum rate is expressed as [76]:
...
$$C^{DCMC}(P_t) = \frac{BW}{\frac{M}{2} - 1} \frac{1}{\mathcal{L}} E \left\{ \sum_{\overline{m}=1}^{\frac{M}{2} - 1} \sum_{\widetilde{l}=0}^{\mathcal{L} - 1} \log_2 \left[ \frac{\mathcal{L} \cdot \exp\left(-\frac{|\overline{y}[n,\overline{m}] - \hat{\overline{h}}_{\overline{m}} \overline{s}^{\widetilde{l}}|^2}{N_0}\right)}{\sum_{\widetilde{l}'=0}^{\mathcal{L} - 1} \exp\left(-\frac{|\overline{y}[n,\overline{m}] - \hat{\overline{h}}_{\overline{m}} \overline{s}^{\widetilde{l}'}|^2}{N_0}\right)} \right] \right\}. (31)$$

<span id="page-8-14"></span><span id="page-8-13"></span><span id="page-8-7"></span><span id="page-8-4"></span>We note that both geometric shaping and probabilistic shaping are capable of further improving the DCMC sum rate by optimizing the non-equal-spaced and non-equi-probable QAM constellations. Interested readers may refer to [77] and [78] for more details.
...
TABLE V
ROOM AND TRANSCEIVER CONFIGURATIONS
...
| LED Coordinates | (1,1,3.4) (1,3,3.4) (1,5,3.4) (1,7,3.4) (3,1,3.4) (3,3,3.4) (3,5,3.4) (3,7,3.4) (5,1,3.4) (5,3,3.4) (5,5,3.4) (5,7,3.4) (7,1,3.4) (7,3,3.4) (7,5,3.4) |  |  |  |  |
|                 | (7,7,3.4)                                                                                                                                             |  |  |  |  |
...
Fig. 19. Communication functionality: Data rates of VLPC-DCO-OFDM in multipath scenario, where we have  $P_t = -20$ dBm. The subfigures are average achievable rates for all LED-User pairs in each  $1 \times 1$   $m^2$  area.
