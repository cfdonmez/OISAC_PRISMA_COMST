...
based on which the expression of far-field light field is recast as  $E(\theta_a, t) \triangleq A_t \mathbf{h} (\theta_a)^{\mathcal{H}} \mathbf{x}(t)$ .
...
$$\Delta\theta_a\left(\vartheta_a\right) = \frac{\sqrt{3}\pi}{N_t k_0 d\cos\left(\vartheta_a\right)}.\tag{5}$$
...
where  $\tilde{L}\left(r_{k}\right)=L_{a}\left(r_{k}\right)\tilde{L}_{t}\left(r_{k}\right)$  is the equivalent total loss, and  $\operatorname{tr}\left(\cdot\right)$  denotes the trace of a matrix. Besides,  $\boldsymbol{R}_{\boldsymbol{w}_{l}}=\boldsymbol{w}_{l}\boldsymbol{w}_{l}^{\mathcal{H}}$ 
...
$$(\boldsymbol{\varrho})_{k} = \operatorname{tr}\left(\boldsymbol{T}_{\theta_{a}}\boldsymbol{H}\left(\theta_{a,k}\right)\right),$$
 (29b)
