...
Cannot justify plane separation or cross-plane conversion \\
\bottomrule
\end{tabularx}
\end{table*}
...
This axis design follows evidence-grounded cross-modality practice and remains compatible with downstream mapping and governance checks \cite{O_ISAC_021}, \cite{O_ISAC_039}, \cite{O_ISAC_077}.
...
\textbf{Table IV-B summarizes medium-based taxonomy classes, corpus share, and dominant operational profiles.}
...
\cite{O_ISAC_006}, \cite{O_ISAC_033}, \cite{O_ISAC_046}, \cite{O_ISAC_041}, \cite{O_ISAC_090} \\
...
\cite{O_ISAC_021}, \cite{O_ISAC_035}, \cite{O_ISAC_005}, \cite{O_ISAC_023}, \cite{O_ISAC_199} \\
...
\cite{O_ISAC_068}, \cite{O_ISAC_303}, \cite{O_ISAC_039}, \cite{O_ISAC_327}, \cite{O_ISAC_062}, \cite{O_ISAC_009} \\
...
Direct (1/1) &
Ranging (1/1) &
\cite{O_ISAC_016}, \cite{O_ISAC_077}, \cite{O_ISAC_105}, \cite{O_ISAC_070}, \cite{O_ISAC_029} \\
...
\cite{O_ISAC_021}, \cite{O_ISAC_077}, \cite{O_ISAC_041}, \cite{O_ISAC_199}, \cite{O_ISAC_010} \\
...
\cite{O_ISAC_039}, \cite{O_ISAC_070}, \cite{O_ISAC_090} \\
\bottomrule
\end{tabularx}
\end{table*}
...
\textbf{Table IV-C summarizes integration mechanisms, evidence intensity, and dominant trade-off implications.}
\begin{table*}[!t]
...
\cite{O_ISAC_035}, \cite{O_ISAC_190}, \cite{O_ISAC_304}, \cite{O_ISAC_016} \\
...
\cite{O_ISAC_021}, \cite{O_ISAC_164}, \cite{O_ISAC_324}, \cite{O_ISAC_056}, \cite{O_ISAC_161} \\
...
\cite{O_ISAC_061}, \cite{O_ISAC_114}, \cite{O_ISAC_142}, \cite{O_ISAC_141}, \cite{O_ISAC_021} \\
...
\cite{O_ISAC_086}, \cite{O_ISAC_166}, \cite{O_ISAC_134}, \cite{O_ISAC_381}, \cite{O_ISAC_161} \\
\bottomrule
\end{tabularx}
\end{table*}
...
\textbf{Table IV-D summarizes detection and observability classes, evidence intensity, and governance implications.}
\begin{table*}[!t]
...
\cite{O_ISAC_001}, \cite{O_ISAC_023}, \cite{O_ISAC_028}, \cite{O_ISAC_029} \\
...
\cite{O_ISAC_006}, \cite{O_ISAC_021}, \cite{O_ISAC_077}, \cite{O_ISAC_190} \\
...
\cite{O_ISAC_021}, \cite{O_ISAC_082}, \cite{O_ISAC_039}, \cite{O_ISAC_056} \\
...
\cite{O_ISAC_021}, \cite{O_ISAC_082}, \cite{O_ISAC_056} \\
...
Retain explicit tail labels and do not silently absorb them into direct/coherent bins &
\cite{O_ISAC_132}, \cite{O_ISAC_061}, \cite{O_ISAC_050} \\
\bottomrule
\end{tabularx}
\end{table*}
...
Evidence-conditioned auditing confirms that this governance is necessary rather than optional. The contract-violation audit reports 84 flagged records in total, split into 75 metric-aliasing cases and 9 measurement-plane ambiguity cases. However, these records are retained for transparency and interpreted conservatively, because removing them would artificially increase apparent agreement at the cost of traceability. Consequently, Section IV-D conclusions prioritize defensible comparability over maximal aggregation: claims are pooled only when plane labels are explicit and metric roles are non-aliased \cite{O_ISAC_132}, \cite{O_ISAC_061}, \cite{O_ISAC_013}, \cite{O_ISAC_050}, \cite{O_ISAC_056}.
...
\cite{O_ISAC_006}, \cite{O_ISAC_041}, \cite{O_ISAC_013} \\
...
\cite{O_ISAC_021}, \cite{O_ISAC_023}, \cite{O_ISAC_199} \\
...
\cite{O_ISAC_068}, \cite{O_ISAC_039}, \cite{O_ISAC_009} \\
...
\cite{O_ISAC_077}, \cite{O_ISAC_010}, \cite{O_ISAC_190} \\

Residual low-support cluster &
Specialized or emerging deployment cases &
Mixed mechanisms &
Mostly direct, with sparse alternatives &
Heterogeneous low-support tasks &
Context-dependent &
Context-dependent &
\cite{O_ISAC_070}, \cite{O_ISAC_056} \\
\bottomrule
\end{tabularx}
\end{table*}
...
-- &
All OSNR-bearing records are blocked by plane mixing in this release \\
\bottomrule
\end{tabularx}
\end{table*}
...
0/161 records &
-- &
$\Delta z$ is retained as fiber-specific context and is never substituted for $\Delta r_{\min}$ \\
\bottomrule
\end{tabularx}
\end{table*}
...
\texttt{resource\_division} 19; \texttt{joint\_waveform} 6; unlabeled 2 \\
\bottomrule
\end{tabularx}
\end{table*}
...
\subsection{VI-A. Programmable Optical Enablers}
...
\subsection{VI-B. Channel Impairments and Robustness}
...
\subsection{VI-C. Joint Co-Design and Resource Optimization}
...
\subsection{VI-D. Experimental Validation, Benchmarking, and Reporting Contract}
...
\subsection{VI-E. Networked and Multi-User O-ISAC}
...
\subsection{VI-F. AI/ML and Security-Aware Adaptation}
...
29.99 Mbps and BER below the 7\% pre-FEC threshold at 500 kHz Doppler &
Range-resolution behavior better than 0.146 m &
Conventional &
\cite{O_ISAC_187} \\

VII-E &
Multi-beam satellite payload &
BER ($8.15 \times 10^{-7}$), EVM (6.74\%), and a 2.4 Gbps 16-QAM link &
Range resolution (14.9 cm) with remote-sensing and imaging descriptors &
Conventional &
\cite{O_ISAC_195} \\
\bottomrule
\end{tabularx}
...
\textbf{Raw only} &
\textbf{Strict only} &
\textbf{\shortstack{Representative\\Reference}} &
