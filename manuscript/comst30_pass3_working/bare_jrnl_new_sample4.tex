...
\begin{IEEEkeywords}
    Optical integrated sensing and communication, O-ISAC, free-space optics, visible light communication, distributed fiber sensing, photonic-THz systems, systematic review.
\end{IEEEkeywords}
...
\begin{table*}[!t]
    ...
    \begin{tabularx}{\textwidth}{@{}L{1.25cm}L{0.58cm}L{1.55cm}Y c c c c c@{}}
        \toprule
...
The optical spectrum supports dense multiplexing well beyond RF-sized blocks. In wireless settings, MDM combined with WDM has realized \textbf{64 parallel channels} (16 wavelengths $\times$ 4 OAM modes) ; in cabled settings, 7-core SDM fiber has reached \textbf{241.85 Tbps} over 38 km . Coexistence-oriented implementations further show that shared optical pipelines can preserve communication throughput while embedding sensing through polarization multiplexing and DSP-compatible integration . Together, these results validate optical parallelism beyond single-link RF limits.

\paragraph{Advantage 2: Enhancing Sensing Precision via Ultra-Wide Bandwidth}
Range resolution in two-way sensing is bandwidth-limited as $\Delta r_{\min} = v/(2B_{\text{eff}})$. Optical bandwidths therefore enable wireless ranging regimes that narrowband RF systems struggle to match: photonic-THz demonstrations report \textbf{2.5 mm} resolution alongside 120 Gbps, giving $\text{CRQ}_{\Delta} = 4.8\times10^{13}$ bps/m . Throughout this review, we keep this bandwidth-limited resolution distinct from SNR-dependent estimation accuracy such as RMSE/$\sigma_r$; fiber DAS contributes through sensitive distributed monitoring rather than fine free-space range resolution .

\paragraph{Advantage 3: Spatial Isolation and Reduced RF-EMI Coupling}
The high directionality of optical beams provides deployment-dependent spatial isolation and can reduce multi-user interference relative to wide-beam RF sectors in clear line-of-sight settings . Wireless O-ISAC still faces turbulence and ambient-light sensitivity , but its weak coupling to conventional RF electromagnetic interference and milliradian-scale beam divergence support dense spatial reuse when propagation conditions are favorable.

\paragraph{Unified O-ISAC Taxonomy}

Having established the optical advantages, we next delimit the scope of this review. O-ISAC is not a single monolithic system class; it clusters into modality families shaped by propagation medium, signal model, and dominant impairment. Section~\ref{sec:taxonomy} develops the evidence-based taxonomy from our PRISMA-compliant corpus, centered on 2020--2025 while retaining a small number of earlier protocol-eligible foundational studies: fiber, free-space optical, VLC/LiFi, and photonic-THz, each with distinct operating windows and sensing--communication conventions.
...
\textbf{Terminology Proliferation.} Closely related ideas are labeled differently across communities: RF-oriented work still uses aliases such as RadCom and JRC , VLC papers alternate between JCS, ISAC, and SCI , visible-spectrum studies may foreground the optical band or control function rather than a shared ISAC vocabulary , and fiber work diverges into labels such as ISAC-OF, fiber-ISAC, and photonic ISAC . This aliasing complicates systematic discovery, inflates perceived novelty through re-labeling, and obscures conceptual links across modalities.
...
where $v=c$ in free space and $v\approx c/n_g$ in guided media. In contrast, RMSE/$\sigma_r$ and localization error remain SNR- and estimator-dependent, while CRB/FIM quantities describe information bounds under an explicit observation model . The same "resolution" label can therefore refer to different objects across modalities, and signal quality also shifts across reference planes: coherent fiber studies often report OSNR , whereas VLC studies usually report post-detection electrical SNR .

\textbf{Sub-Domain Siloing and Limited Cross-Citation.} Fiber DAS/communication co-design, hybrid fiber communication/sensing network planning, FSO ranging links, VLC positioning/data systems, and photonic-THz ISAC have largely evolved in separate channel, hardware, and evaluation cultures . As a result, a trade-off frontier in one modality is rarely comparable to another without harmonized assumptions. Recent assessments echo this separation, noting weak integration even within VLC/VLP ecosystems and persistent interoperability barriers across sectors .

\textbf{Weak Cross-Domain Technology Transfer.} Methods developed in one optical sub-domain are only weakly reused in others. Fiber-oriented waveform/probing structures seldom transfer to free-space or VLC links, and VLC-centric localization or learning pipelines rarely reappear in FSO or photonic-THz settings . The same pattern appears in application-facing designs, where UAV semantic communication, logistics orchestration, and vehicular path-loss adaptation are usually developed within narrow deployment silos rather than as reusable cross-modality building blocks . Without shared benchmarks and reporting protocols, it remains difficult to separate what is modality-specific from what is genuinely transferable .

\textbf{The Missing Unifying Framework.} These issues point to the same gap: O-ISAC still lacks a shared physical-layer taxonomy, a standardized reporting contract, and a cross-modality benchmark culture. At minimum, comparable studies should disclose communication performance, sensing performance separated into $\Delta r_{\min}$, estimator-level error, and CRB/FIM-style bounds, signal quality at an explicit reference plane, and the channel/scenario assumptions that govern comparability. Recent calls for standardized VLC channel models and interoperability-oriented standardization reinforce the urgency of this unification .

\paragraph{Related Surveys and Gap Analysis}

RF-ISAC already has a mature survey landscape, but optical ISAC remains comparatively underserved. Existing review-style work is still modality-centric: VLC surveys emphasize positioning and indoor propagation , fiber reviews focus on DFOS/DAS integration and co-route monitoring , and FSO or photonic-THz surveys discuss architectures, waveforms, and high-CRQ demonstrations without bridging the full FSO--fiber--VLC divide . Parallel primary studies continue to deepen individual silos, for example in movable VLC access, sensing-front-haul coexistence, and W-band fiber-wireless integration, but they still do not add up to a unified cross-modality survey layer . Within our 220-study corpus, only the VLC-based LiSAC review explicitly treats optical ISAC as its primary topic, and even that work remains single-modality and non-PRISMA . RIS-for-ISAC surveys provide useful enabling-technology context but remain RF/THz-centric. Table~\ref{tab:axis_comparison} summarizes this landscape.
...
\begin{table*}[!t]
    ...
    \renewcommand{\arraystretch}{1.08}
    \begin{tabularx}{\textwidth}{@{}L{1.65cm}L{2.05cm}L{2.15cm}L{2.0cm}Y@{}}
        \toprule
        Modality & Channel basis & Receiver/observable & Typical task & Governance warning \\
        \midrule
        Fiber & Guided dispersive/nonlinear link; DAS/DFOS granularity & Coherent or direct optical/electrical plane & Vibration, strain, co-route monitoring & Report $\Delta z$ and OSNR plane; do not call it wireless range resolution \\
        ...
        VLC/LiFi & Lambertian LoS/NLoS indoor optical wireless & Mostly IM/DD; sometimes camera-assisted & Localization and indoor sensing & Electrical SNR/RMSE cannot be merged with optical OSNR or CRB without model disclosure \\
        Photonic-THz/hybrid & Optical generation/distribution plus mmWave/THz links & Coherent photonic, electronic, or hybrid chains & High-rate ranging and fiber-wireless transfer & Separate optical generation, wireless propagation, and post-detection planes \\
        \bottomrule
    \end{tabularx}
\end{table*}
...
\begin{table*}[!t]
    ...
    \renewcommand{\arraystretch}{1.08}
    \begin{tabularx}{\textwidth}{@{}L{2.15cm}L{2.15cm}L{2.7cm}Y@{}}
        \toprule
        ...
        OSNR/SNR/ESNR & Signal-quality plane & optical/electrical reference point and bandwidth & Mixing planes without receiver/noise model \\
...
Table~\ref{tab:iii1} summarizes the eligibility boundary used to preserve optical relevance while retaining the field's fragmented terminology.
...
The complete 220-study corpus is a systematic-review asset, but printing all 220 records in the main bibliography is not the best use of a COMST page budget. The main text therefore cites representative and load-bearing studies, while the full corpus remains traceable through the included-corpus ledger, extraction sheet, and supplementary evidence records. This separation protects systematic-review reproducibility while keeping the main bibliography focused on studies that directly support the narrative, taxonomy, enablers, applications, or governed metric synthesis.
...
Fig.~\ref{fig:fig_iv_1} is retained as the single main-text taxonomy map; Table~\ref{tab:taxonomy_compact} gives the compact corpus-level summary.
...
\begin{table*}[!t]
    ...
    \setlength{\tabcolsep}{2.7pt}
    \renewcommand{\arraystretch}{1.08}
    \begin{tabularx}{\textwidth}{@{}L{2.0cm}L{1.65cm}L{2.25cm}L{2.2cm}Y@{}}
        \toprule
        ...
        Photonic-THz / hybrid & 116/220 hybrid; 39 photonic-THz anchors & Near-balanced coherent/direct & Ranging-heavy, multi-task tail & Separate optical generation/distribution from wireless propagation and receiver plane \\
        \bottomrule
    \end{tabularx}
\end{table*}
...
This operational contract binds axis assignment, deterministic mapping, and comparability protection.
...
Representative hardware-centric records include heterogeneous III-V-on-SOI transmitters and monolithic III-nitride chip/circuit realizations, which make the integration claim concrete at device and photonic-platform level rather than only at block-diagram level .

Shared resources is the most explicit mechanism class, with 43 unique papers under combined support and 16 direct-only anchors. Its high support is consistent with VLC/LiFi and FSO settings where integration often occurs through time/frequency/power coordination rather than full hardware unification, but the mechanism also appears in shared-front-end-dominant classes. It should therefore be read as a control-plane mechanism spanning both co-designed and coexistence architectures, with outcomes governed mainly by contention and scheduling quality .
The same control-plane interpretation is visible in low-altitude optical backbone studies, AI-enhanced fiber-wireless mobility support, and confluent edge-network coordination, where integration value is expressed through resource orchestration and network-state adaptation rather than through a single shared transceiver chain .

Shared processing has 30 unique papers under combined support and 8 direct-only anchors. It is less explicit than shared resources but remains materially represented, especially in hybrid-dominant settings where multi-stage pipelines benefit from joint inference. Its main contribution is algorithmic coupling across decoding and sensing inference, but any reported gain still depends on transparent model assumptions and robustness to task interference .
Recent processing-centric examples include multimodal perception fusion for beamforming, RF-to-LiDAR digital-twin inference, and learning-assisted photonics W-band ISAC reception, which all treat shared inference as a first-class integration layer rather than a downstream add-on .
...
\begin{table*}[!t]
    ...
    \setlength{\tabcolsep}{3pt}
    \renewcommand{\arraystretch}{1.08}
    \begin{tabularx}{\textwidth}{@{}L{2.8cm}L{2.0cm}L{2.2cm}Y@{}}
        \toprule
...
\begin{table*}[!t]
    ...
    \setlength{\tabcolsep}{2.8pt}
    \renewcommand{\arraystretch}{1.08}
    \begin{tabularx}{\textwidth}{@{}L{1.7cm}L{2.0cm}L{2.1cm}Y@{}}
        \toprule
        Slice & Strong evidence & Weakness for pooling & Main reading \\
        \midrule
        Fiber & Tb/s-class links and distributed sensing \cite{O_ISAC_046} & $\Delta z$ and OSNR differ from wireless range/ESNR & Guided infrastructure anchor \\
        Wireless optical & FSO/VLC ranging/localization and OPA links \cite{O_ISAC_023,O_ISAC_061} & Geometry, turbulence, ambient light, and estimator differences & Scenario-conditioned tradeoff \\
...
\begin{table*}[!t]
    ...
    \setlength{\tabcolsep}{2.8pt}
    \renewcommand{\arraystretch}{1.08}
    \begin{tabularx}{\textwidth}{@{}L{2.15cm}L{2.65cm}L{2.3cm}Y@{}}
        \toprule
        Enabler & Main O-ISAC role & Evidence center & Reporting risk \\
        \midrule
        ORIS / optical RIS & Programmable reflection, beam control, spatial reuse & Optical wireless and RIS-adjacent ISAC \cite{O_ISAC_163} & Disclose geometry and control overhead \\
        OPA & Beam steering and joint aperture use & OPA-based optical wireless ISAC \cite{O_ISAC_061,O_ISAC_091} & Link beam pattern, scan schedule, and estimator \\
        PIC / photonic integration & Compact source/modulator/receiver integration & Fiber-wireless and chip-scale demonstrations \cite{O_ISAC_036,O_ISAC_045} & Map device results to system metric planes \\
        Photonics-assisted mmWave/THz & High-frequency generation and reception & W-band, D-band, and THz-over-fiber systems \cite{O_ISAC_029,O_ISAC_070} & Separate optical generation from wireless propagation \\
        ML/security adaptation & Inference, calibration, and attack-aware control & Learning-assisted VLC/fiber-wireless studies \cite{O_ISAC_039,O_ISAC_242} & Disclose dataset, generalization, and threat model \\
        \bottomrule
    \end{tabularx}
\end{table*}
...
AI-assisted adaptation and security-aware design are emerging layers in O-ISAC, especially in dynamic channels where static policies may underperform. Repository tag views disagree on the exact ML prevalence under strict versus broad evidence rules, so this section treats ML as a material but uneven subset rather than a precise maturity count. The available literature contains targeted reports of learning-driven adaptation, learning-assisted compensation and receiver refinement, and learning-based angle estimation, together with secrecy, authentication, and resilience formulations, but jointly validated AI-plus-security optical benchmarks remain limited. In the review flow, this subsection functions as a forward-looking systems layer rather than a co-equal replacement for the physical enabler evidence in VI-A.
...
\begin{table*}[!t]
    ...
    \setlength{\tabcolsep}{2.8pt}
    \renewcommand{\arraystretch}{1.08}
    \begin{tabularx}{\textwidth}{@{}L{2.35cm}L{2.6cm}L{2.55cm}Y@{}}
        \toprule
...
The roadmap is organized around five challenge domains: standardization/interoperability, hardware scalability, channel modeling/evaluation, security/privacy/reliability, and deployment convergence. Fig.~\ref{fig:fig_viii_1} links these domains back to the metric contract, enablers, and application motifs; Table~\ref{tab:challenge_compact} summarizes the challenge set, and Table~\ref{tab:viii_f_2} converts it into a five-item agenda.
...
\begin{table*}[!t]
    ...
    \begin{tabularx}{\textwidth}{@{}L{2.4cm}L{3.0cm}Y@{}}
        ...
        \midrule
        Std./interoperability & Inconsistent terminology, taxonomy, and reporting fields & Shared O-ISAC taxonomy, PRISMA-style ledgers, metric-governed templates \\
        Hardware scalability & Promising but uneven OPA/ORIS/PIC/photonic-THz integration & Co-designed aperture, source, receiver, calibration, and packaging metrics \\
        ...
        Security/reliability & ML, sensing data, and narrow beams add attack/failure surfaces & Threat-modeled evaluation with robustness and privacy fields \\
...
\begin{table*}[!t]
    ...
    \setlength{\tabcolsep}{2.8pt}
    \renewcommand{\arraystretch}{1.08}
    \begin{tabularx}{\textwidth}{@{}L{0.55cm}L{2.7cm}Y L{2.15cm}@{}}
        \toprule
        Pri. & Agenda item & Concrete deliverable & Risk if deferred \\
        \midrule
        1 & Metric-governed reporting & Required fields for rate, $\Delta r_{\min}$, $\sigma_r$/RMSE, CRB/FIM, $\Delta z$, and OSNR/SNR plane & Unsafe comparisons persist \\
