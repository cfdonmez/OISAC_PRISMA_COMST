...
- Across all four scenarios, a conservative Conventional dominant-component label is evidence-consistent because explicit ORIS control-variable formulations are not directly reported in the cited studies [O_ISAC_127], [O_ISAC_220], [O_ISAC_020], [O_ISAC_027].
...
- The scenario vectors are driven by topology and motion conditions that are directly evidenced in the cited studys, including mesh ISLs, LEO mobility context, station-to-satellite links, and multi-beam payload structure [O_ISAC_089], [O_ISAC_137], [O_ISAC_187], [O_ISAC_195].
...
- Evidence-bound synthesis in this subsection remains Conventional at scenario level unless explicit OPA or ORIS dominance is directly reported in cited deployment studies [O_ISAC_074] [O_ISAC_187] [O_ISAC_164] [O_ISAC_011] [O_ISAC_108].
...
Case 2, indoor mixed contraction (`indoor_environments`): View-1 records `study_flag_count=65`, while View-2 reports `raw_evidence_count=175` and `strict_evidence_count=81`. The discrepancy contracts from `raw_only_vs_flag=110` to `strict_only_vs_flag=16` under strict gating (comparison row #3). Representativeness is anchored in the indoor `strict_only` examples set (examples row #7), which includes `O_ISAC_013` [O_ISAC_013]. The pattern indicates that raw extraction substantially over-expands indoor coverage, yet strict filtering still preserves a modest support-qualified surplus beyond structured tags.
