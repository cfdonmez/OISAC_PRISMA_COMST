...
\toprule
\thead{Element} & \thead{Role in taxonomy} & \thead{Operational rule} & \thead{Comparability guard} \\
\midrule
...
\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{\textwidth}{@{}L{1.9cm}C{1.7cm}Y Y Y L{2.5cm}@{}}
\toprule
\thead{Medium class} & \thead{Corpus\\share} & \thead{Dominant integration\\profile} & \thead{Dominant detection\\profile} & \thead{Dominant sensing\\emphasis} & \thead{Representative\\studies} \\
\midrule
Fiber (cabled) & 45/220 (20.5\%) & Shared front-end (43/45) & Coherent-leading (27 coherent, 18 direct) & Ranging primary (27), then vibration (8) and 2D localization (6) & \cite{O_ISAC_006}, \cite{O_ISAC_033}, \cite{O_ISAC_046}, \cite{O_ISAC_041}, \cite{O_ISAC_090} \\
FSO & 19/220 (8.6\%) & Shared front-end (14/19) & Direct/coherent mixed with direct lead (11/8) & Ranging primary (18), 2D localization tail (1) & \cite{O_ISAC_021}, \cite{O_ISAC_035}, \cite{O_ISAC_005}, \cite{O_ISAC_023}, \cite{O_ISAC_199} \\
VLC/LiFi & 25/220 (11.4\%) & Shared front-end dominant (18/25) & Direct-detection dominant (24/25) & Ranging (12) and localization-heavy tail (10 2D localization, 3 localization) & \cite{O_ISAC_068}, \cite{O_ISAC_303}, \cite{O_ISAC_039}, \cite{O_ISAC_327}, \cite{O_ISAC_062}, \cite{O_ISAC_009} \\
Photonic-THz / terahertz proxy & 1/220 (0.5\%) explicit terahertz class & Shared front-end (1/1) & Direct (1/1) & Ranging (1/1) & \cite{O_ISAC_016}, \cite{O_ISAC_077}, \cite{O_ISAC_105}, \cite{O_ISAC_070}, \cite{O_ISAC_029} \\
Hybrid systems & 116/220 (52.7\%) & Shared front-end dominant (106/116) & Near-balanced coherent/direct (59/53) with residual labels (4) & Ranging primary (95), 2D localization tail (12), broader multi-task remainder & \cite{O_ISAC_021}, \cite{O_ISAC_077}, \cite{O_ISAC_041}, \cite{O_ISAC_199}, \cite{O_ISAC_010} \\
Long-tail media (outside main five) & 14/220 (6.4\%) & Mixed, mostly shared front-end & Mostly direct-detection & Heterogeneous low-support tasks & \cite{O_ISAC_039}, \cite{O_ISAC_070}, \cite{O_ISAC_090} \\
\bottomrule
\end{tabularx}
\end{table}
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.12}
\begin{tabularx}{\textwidth}{@{}L{2.5cm}L{2.2cm}Y Y L{2.5cm}@{}}
\toprule
\thead{Detection /\\observability class} & \thead{Evidence\\intensity} & \thead{Corpus-level anchor} & \thead{Governance implication} & \thead{Representative\\references} \\
\midrule
IM/DD regime & 29 unique papers (direct+indirect); direct-only 23 & Direct-detection-heavy VLC/LiFi plus direct subsets in FSO and fiber & Interpret under intensity-domain and post-detection electrical-plane semantics; no implicit OSNR-to-SNR transfer & \cite{O_ISAC_001}, \cite{O_ISAC_023}, \cite{O_ISAC_028}, \cite{O_ISAC_029} \\
Coherent detection & 45 unique papers (direct+indirect); direct-only 25 & Fiber and hybrid anchors with coherent-leading subclusters & Preserve receiver/noise-model transparency before cross-regime comparison & \cite{O_ISAC_006}, \cite{O_ISAC_021}, \cite{O_ISAC_077}, \cite{O_ISAC_190} \\
Intensity-only observability & 118 unique papers (direct+indirect); direct-only 58 & Dominant in direct-detection corpora and especially VLC/LiFi & Do not infer phase-sensitive or complex-field behavior from intensity-only evidence & \cite{O_ISAC_021}, \cite{O_ISAC_082}, \cite{O_ISAC_039}, \cite{O_ISAC_056} \\
Complex-field observability & 17 unique papers (direct+indirect); direct-only 4 & Coherent-leading fiber/hybrid subset & Keep field-aware estimator behavior separate from intensity-only aggregation & \cite{O_ISAC_021}, \cite{O_ISAC_082}, \cite{O_ISAC_056} \\
Residual receiver labels / structured tail & 5 residual corpus labels; structured receiver typing populated in 218/220 studies & Unknown, envelope-detection, MIMO, and other low-support labels & Retain explicit tail labels and do not silently absorb them into direct/coherent bins & \cite{O_ISAC_132}, \cite{O_ISAC_061}, \cite{O_ISAC_050} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tabularx}{\textwidth}{L{1.75cm} Y Y Y Y L{1.65cm} L{1.7cm} L{2.1cm}}
\toprule
...
\midrule
Fiber-guided O-ISAC cluster & Long-reach cabled links with distributed monitoring overlays & Shared front-end dominant & Coherent-leading with direct subset & Ranging primary, vibration concentration, and a fault-localization tail & BER, FEC margin, communication robustness & Spatial granularity/ranging and vibration indicators & \cite{O_ISAC_006}, \cite{O_ISAC_041}, \cite{O_ISAC_013} \\
FSO wireless optical cluster & Line-of-sight outdoor and inter-building optical links & Shared front-end with a non-negligible separate-front-end subset & Direct/coherent mixed & Ranging dominant with a small localization tail & BER, outage, spectral efficiency & Range estimation and resolution-class indicators & \cite{O_ISAC_021}, \cite{O_ISAC_023}, \cite{O_ISAC_199} \\
VLC/LiFi indoor cluster & Illumination-constrained indoor access and positioning & Shared front-end dominant & Direct-detection and intensity-only dominant & Ranging plus strong localization concentration & Throughput, BER, link reliability & Localization error and ranging metrics & \cite{O_ISAC_068}, \cite{O_ISAC_039}, \cite{O_ISAC_009} \\
Hybrid bridge cluster & Fiber-wireless and photonic-THz bridging pipelines & Shared front-end dominant & Near-balanced coherent/direct with residual labels & Ranging dominant with a multi-task tail & Throughput and BER under cross-link coupling & Range/localization and task-specific errors & \cite{O_ISAC_077}, \cite{O_ISAC_010}, \cite{O_ISAC_190} \\
Residual low-support cluster & Specialized or emerging deployment cases & Mixed mechanisms & Mostly direct with sparse alternatives & Heterogeneous low-support tasks & Context-dependent & Context-dependent & \cite{O_ISAC_070}, \cite{O_ISAC_056} \\
\bottomrule
\end{tabularx}
\end{table}
