...
\subsection{Applying the Comparison Profile to Individual Metrics}

Applying the comparison profile defined in
Section~\ref{sec:foundations_comparison}, each metric row was coded by source
definition and unit, task, measurement plane, operating point, available
baseline, validation method, and value provenance, with its report and source
location retained. Communication and sensing results form a joint operating
...
supported interpretation without creating additional numerical measurements.
Source conflicts were assessed at the individual-claim level, preserving
supported findings from the same study. Electronic Supplement S-Protocol
...
complete metric register contains 4,861 rows. The additional rows comprise 31
contextual records and 51 records documenting source conflicts. Comparison
follows the reported definition, task, measurement plane, conditions, and
validation. Electronic Supplement S-Evidence provides the complete records and
provenance, including quantities grouped under the broader other category.}
\end{table}
...
stated denominator. In a terahertz prototype enabled by photonics, net rates
...
fiber experiment over a long span reports receiver OSNR and BER before correction
...
The synthesis covers 402 relationships from 168 studies.
Figure~\ref{fig:tradeoff_profile} separates record type, overlapping study
coverage, and condition dependence across 11 recurring mechanism families.
Each relationship remains tied to its waveform, geometry, target, observation
point, and validation setting.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/final_submission_2026-08-25/fig06_tradeoff_profile.pdf}
\caption{Evidence profile for 402 substantive joint performance relationships
from 168 studies. Panel (a) separates 218 quantitative and 184 qualitative
records. Panel (b) reports overlapping study coverage across the mechanism
families. Panel (c) reports the share of records whose interpretation depends
on stated conditions, with 371 such relationships across the profile. Family
study counts may overlap, and frequency describes corpus coverage.}
\label{fig:tradeoff_profile}
\end{figure*}

Resource and power mechanisms carry much of the coded evidence, while condition
dependence remains common across the families. This pattern directs the
discussion toward the shared mechanism and the operating condition that shapes
each reported relation.
...
BER or error vector magnitude (EVM), and radar or ranging quality within the
available receiver headroom
...
and receiver are selected. Communication evidence uses BER, EVM, quality
factor, and required OSNR, while sensing evidence uses SNR, estimation error,
and detection.
...
wave waveform, a direct current offset strengthens the sensing component and
raises the radar signal to interference ratio together with communication EVM
\cite{OISAC_SCR00808}. The first mechanism changes how
...
reported communication and sensing metrics. Carrier choice, direct current
offset, and phase
...
\cite{OISAC_SCR00009,OISAC_SCR00196}. Orthogonal time frequency space
parameterization and complete grid
...
sensing fidelity \cite{OISAC_SCR00387,OISAC_SCR00647}. Amplitude shift keying
rate and guard
...
device band supports quadrature phase shift keying (QPSK) communication and the
LFM bandwidth used to achieve
range resolution
...
nonorthogonal multiple access power allocation connects aggregate rate with
ranging error
...
Across the network, the relationship moves from a single waveform to the traffic
...
complexity in the optical source chain \cite{OISAC_SCR01000}. A fiber experiment
over a long span uses lower cost lasers with broad linewidths and recovers coherent
