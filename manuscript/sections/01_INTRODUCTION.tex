...
We conducted a systematic literature review, with reporting guided by
PRISMA 2020 and PRISMA-S \cite{PRISMA2020,PRISMAS2021}.
Searches covered six bibliographic and publisher sources and were completed
on 22 June 2026. The publication window extended from 1 January 2020 to that
date, using the date when a report first became publicly available.
Search strategies combined O-ISAC terminology with terms for joint sensing
and communication in optical or photonic systems. Eligible reports were
peer-reviewed journal articles or full conference papers describing
communication and sensing within a shared optical or photonic system,
with sufficient English-language technical content to assess at least one
survey domain.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/selection.pdf}
\caption{Study selection for the O-ISAC review. The flow distinguishes search
records, eligible reports, and underlying studies. The 227 eligible reports
describe 206 studies; 21 companion reports remain linked to those studies.
The 67 contextual records support background and field positioning.
Detailed eligibility decisions are provided in Electronic Supplement
S-Exclusions.}
\label{fig:prisma_report_study_flow}
\end{figure*}

After duplicate removal, records underwent title and abstract screening
followed by full-text eligibility assessment.
Figure~\ref{fig:prisma_report_study_flow} summarizes the selection flow and
distinguishes publications from underlying studies. Contextual literature
was retained separately for background and field positioning.
We extracted reported results with their metric definitions, units,
measurement points, and operating conditions, and used structured narrative
synthesis to examine recurring mechanisms and performance relations.
Numerical comparisons retained the conditions under which each result was
obtained. The search strategies, eligibility criteria, study-selection
records, and supporting review materials are available in the OSF
repository.\footnote{\url{https://osf.io/7f6wb/}}
...
\textbf{A physical basis for selecting an O-ISAC design.}
...
concepts used in the review. Sections~\ref{sec:optical_modality_families}
