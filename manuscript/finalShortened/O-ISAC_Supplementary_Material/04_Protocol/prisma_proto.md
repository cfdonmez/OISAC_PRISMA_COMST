...
The protocol is registered on the **Open Science Framework (OSF)** under a dedicated project for the optical ISAC (O-ISAC) systematic review. The OSF registration ID is `7f6wb` (registered on February 12, 2026). The registration is accessible at https://osf.io/7f6wb.
...
To capture both the communication-theoretic and optoelectronic physical-layer aspects of optical integrated sensing and communication (O-ISAC), the review uses a clearly separated two-tier source policy: (i) a **formal PRISMA identification stage** based on canonical bibliographic databases, and (ii) **supplementary monitoring sources** used for version tracing and completeness checks. All executed sources, search strategies, and search dates are documented in detail in line with PRISMA 2020 and the PRISMA-S extension for reporting literature searches.

### 5.1 Electronic Databases

For the current review freeze, the formal PRISMA identification stage was executed using the following three databases, all last searched on **2025-11-30**:
...
The three databases above constitute the canonical formal identification stage for the current PRISMA flow.

4. **Optional Optica Publishing Group (formerly OSA) follow-up**
...
5. **Optional SPIE Digital Library follow-up** 
...
These publisher-platform checks are supplementary procedures. They are not part of the canonical PRISMA identification totals for the current freeze unless separately executed, logged, and added to the flow counts.
...
Preprint servers are treated as **supplementary monitoring sources** for the current freeze. They support early-version tracing and future updates but do not define the canonical PRISMA identification totals unless separately logged:
...
Preprints will be **tracked and documented** but, consistent with the eligibility criteria (Section 4.5), only peer-reviewed or formally accepted versions will be included in the primary evidence base whenever such versions are available. For the current freeze, no supplementary-source records were retained as separate additions to the canonical PRISMA flow; accordingly, `other_sources_results = 0` in the final flow counts.
...
To minimise the risk of missing relevant O-ISAC studies, the following supplementary search methods may be used as auxiliary checks:
...
The details of each supplementary search (venue, years covered, access route, and date searched) will be recorded when executed. For the current freeze, no separately logged additions from these supplementary methods were retained in the canonical PRISMA identification totals.

### 5.4 Documentation and Transparency

For every executed information source (formal databases and any supplementary sources actually used), the following will be reported in the final review:
...
<!-- Canonical formal search log: `search/search_log.csv`; formal databases last searched on 2025-11-30. -->
...
The search strategy is designed to retrieve optical integrated sensing and communication (O-ISAC) studies across both cabled (fibre-based) and wireless (FSO/VLC/LiDAR-like) domains, with explicit emphasis on physical-layer architectures, signal and channel models, and joint sensing–communication performance. Search methods will be planned and reported in accordance with PRISMA 2020 and the PRISMA-S extension for reporting literature searches. For the current freeze, the formal PRISMA identification stage was executed via IEEE Xplore, Scopus, and Web of Science, while supplementary monitoring templates were retained separately for non-canonical sources.
...
Database-specific syntax (field tags, proximity operators, wildcards, and filters) is used for each executed formal source listed in Section 5. The full, exact search strings for each canonical database are archived in `search/search_strings.md` to ensure reproducibility.
...
#### 6.2.3 Optional Publisher-Platform Follow-up Checks

Publisher-platform follow-up checks (e.g., Optica or SPIE interfaces) may be used as optional supplementary procedures when database indexing gaps are suspected. These checks are not part of the canonical formal identification stage unless they are separately executed, logged, and reflected in the PRISMA flow.

#### 6.2.4 Preprint Monitoring Template (arXiv, TechRxiv)

For arXiv (categories eess.SP, cs.IT, physics.optics) and TechRxiv, simplified monitoring queries may be used, for example:
...
Searches will be filtered by subject area when possible and limited to English-language manuscripts. These monitoring templates are supplementary and are not included in the primary PRISMA identification totals for the current freeze unless separately logged.
...
For the current freeze, the canonical formal database searches were last run on **2025-11-30**. Full search strategies for all executed formal databases, together with any supplementary source documentation actually used, will be provided in an appendix or supplementary file and deposited in an open repository together with the search log.
...
- **Standardised Exclusion Coding:** Records excluded at full-text stage will be assigned a predefined exclusion code to enable transparent reporting and quantitative breakdowns in the PRISMA flow, for example:
  ...
  - `EXC-UNVERIFIED-FULLTEXT`: Bibliographic record identified, but the frozen evidence package does not contain a verified full-text asset for reliable extraction or inclusion audit.
...
All screening decisions and full-text exclusion reasons will be archived in structured CSV artefacts (e.g., `search/dedup_log.csv`, `screening/title_abstract_screening_reconstruction.csv`, `screening/screening_log.csv`, and `screening/excluded_fulltext_log.csv`) and version-controlled in the repository, enabling independent verification and future updating of the review.
...
- screening artefacts (e.g., `search/dedup_log.csv`, `screening/title_abstract_screening_reconstruction.csv`, `screening_log.csv`, and `excluded_fulltext_log.csv` with inclusion/exclusion decisions and exclusion codes),
