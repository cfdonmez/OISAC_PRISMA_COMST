...
\label{introduction}
\paragraph{The Convergence of Sensing and Communication: A 6G Imperative}
The escalating complexity of the electromagnetic environment has intensified demand for wireless systems that can communicate and sense jointly. ISAC is therefore now treated as a core 6G enabler and is recognized by both the ITU-R IMT-2030 framework and 3GPP as part of the future service landscape \cite{O_ISAC_070,O_ISAC_162}. Emerging applications such as robot navigation, augmented reality, autonomous driving, and human--machine interaction increasingly require high-rate communication and high-accuracy environmental awareness in the same workflow \cite{O_ISAC_016,O_ISAC_351}. Recent optical studies already extend this application space to cooperative mobile robotics, gesture-aware intelligent surfaces, vehicular optical links, and drone-assisted localization workflows \cite{O_ISAC_084,O_ISAC_123,O_ISAC_129,O_ISAC_143}. Yet even with strong progress in RF-based ISAC, the migration toward mmWave and THz does not remove three persistent constraints:
...
\textbf{Optical Integrated Sensing and Communication (O-ISAC)} addresses these limits by unifying perception, transmission, and processing on optical carriers \cite{O_ISAC_021}. As summarized in Fig.~\ref{fig:fig1}, optical carriers open access to the 28.3--845 THz design space and naturally branch into fiber, free-space, VLC/LiFi, and optical--THz-bridging settings. Across the 220-study corpus considered here, representative prototypes already report \textbf{120 Gbps wireless throughput with 2.5 mm sensing resolution} in photonic-THz integration \cite{O_ISAC_105} and \textbf{241.85 Tb/s aggregate transmission with concurrent sensing} in seven-core fiber systems \cite{O_ISAC_046}. Beyond these headline results, recent work also demonstrates fiber sensing-telecommunication integration, photonics-assisted W-band fiber-wireless ISAC, and real-time photonic sub-THz O-ISAC links \cite{O_ISAC_004,O_ISAC_043,O_ISAC_111}. These results motivate the central question of this review: which physical properties of the optical domain enable such gains, and how can they be synthesized under a defensible cross-modality framework?
...
\paragraph{The Optical Opportunity: A Vast and Untapped Frontier}
The optical domain expands the design space far beyond congested sub-300 GHz RF bands, nominally spanning approximately \textbf{28.3 THz to 845 THz}, though practical systems cluster in atmospheric transmission windows and fiber telecom bands \cite{O_ISAC_021}. In this review, Photo-THz O-ISAC denotes photonics-enabled architectures in which optical carriers support generation/LO/distribution while wireless propagation occurs in the sub-THz/THz band. Recent system-level studies also place O-ISAC within transport-supported, terrestrial/non-terrestrial, and context-aware optical-network architectures rather than only isolated link demonstrations \cite{O_ISAC_059,O_ISAC_088,O_ISAC_101}. The following paragraphs isolate the physical advantages that make that spectrum attractive for next-generation ISAC.
\paragraph{Quantitative Comparison: RF-ISAC vs. O-ISAC}
...
\paragraph{Three Competitive Advantages of O-ISAC}
...
The optical spectrum supports dense multiplexing well beyond RF-sized blocks. In wireless settings, MDM combined with WDM has realized \textbf{64 parallel channels} (16 wavelengths $\times$ 4 OAM modes) \cite{O_ISAC_021}; in cabled settings, 7-core SDM fiber has reached \textbf{241.85 Tbps} over 38 km \cite{O_ISAC_046}. Coexistence-oriented implementations further show that shared optical pipelines can preserve communication throughput while embedding sensing through polarization multiplexing and DSP-compatible integration \cite{O_ISAC_080,O_ISAC_097,O_ISAC_113}. Together, these results validate optical parallelism beyond single-link RF limits.
...
\paragraph{Unified O-ISAC Taxonomy}
...
\paragraph{The Fragmentation Challenge: A Landscape Without Unity}
...
\textbf{Terminology Proliferation.} Closely related ideas are labeled differently across communities: RF-oriented work still uses aliases such as RadCom and JRC \cite{O_ISAC_161}, VLC papers alternate between JCS, ISAC, and SCI \cite{O_ISAC_068}, visible-spectrum studies may foreground the optical band or control function rather than a shared ISAC vocabulary \cite{O_ISAC_053}, and fiber work diverges into labels such as ISAC-OF, fiber-ISAC, and photonic ISAC \cite{O_ISAC_041,O_ISAC_033}. This aliasing complicates systematic discovery, inflates perceived novelty through re-labeling, and obscures conceptual links across modalities.
...
\textbf{Sub-Domain Siloing and Limited Cross-Citation.} Fiber DAS/communication co-design, hybrid fiber communication/sensing network planning, FSO ranging links, VLC positioning/data systems, and photonic-THz ISAC have largely evolved in separate channel, hardware, and evaluation cultures \cite{O_ISAC_033,O_ISAC_050,O_ISAC_082,O_ISAC_128}. As a result, a trade-off frontier in one modality is rarely comparable to another without harmonized assumptions. Recent assessments echo this separation, noting weak integration even within VLC/VLP ecosystems and persistent interoperability barriers across sectors \cite{O_ISAC_039,O_ISAC_161}.

\textbf{Weak Cross-Domain Technology Transfer.} Methods developed in one optical sub-domain are only weakly reused in others. Fiber-oriented waveform/probing structures seldom transfer to free-space or VLC links, and VLC-centric localization or learning pipelines rarely reappear in FSO or photonic-THz settings \cite{O_ISAC_042,O_ISAC_074,O_ISAC_022,O_ISAC_039}. The same pattern appears in application-facing designs, where UAV semantic communication, logistics orchestration, and vehicular path-loss adaptation are usually developed within narrow deployment silos rather than as reusable cross-modality building blocks \cite{O_ISAC_094,O_ISAC_155,O_ISAC_160}. Without shared benchmarks and reporting protocols, it remains difficult to separate what is modality-specific from what is genuinely transferable \cite{O_ISAC_068,O_ISAC_067}.
...
\paragraph{Contributions of This Review}
...
\paragraph{Notation and Acronyms}
...
Recent full-duplex microwave-photonic, photonics-enabled millimeter-wave, and simultaneous sensing-communication-over-fiber implementations illustrate how the same abstraction can be instantiated with different sharing depths across wireless and guided settings \cite{O_ISAC_019,O_ISAC_031,O_ISAC_045}.
...
\end{equation}
where $A(z,t)$ is the optical field envelope, $\alpha$ is attenuation, $\beta_2$ is group-velocity dispersion, and $\gamma$ is the nonlinear coefficient. For sensing tasks, fiber reports spatial granularity via $\Delta z$ (gauge/segment length), not wireless-style $\Delta r_{\min}$ (Metric Governance). Integrated fiber O-ISAC implementations also show that probe pre-compensation, temperature compensation, and estimator-side vibration recovery can be central to the effective observability chain in guided media \cite{O_ISAC_024,O_ISAC_032,O_ISAC_072,O_ISAC_085}.
...
Evidence alignment: Representative photonics-assisted mmWave/THz studies discuss multipath and frequency-selective channel effects in fiber-wireless settings \cite{O_ISAC_241,O_ISAC_077}. Waveform-centric W-band and THz-over-fiber O-ISAC studies likewise model fiber-wireless propagation as an integrated optical-plus-wireless chain rather than a single homogeneous link \cite{O_ISAC_047,O_ISAC_065,O_ISAC_119}.
...
Evidence alignment: Representative photonic-THz chains use ECL-driven MZM/IQ-modulator front-ends consistent with coherent operation \cite{O_ISAC_029}. Self-homodyne coherent transmission with LFM-carried sensing further shows how modulation-format choice remains tightly coupled to the front-end interface in integrated optical links \cite{O_ISAC_079}. IM/DD-oriented VLC implementations use LED/LD intensity modulation with nonnegative optical intensity constraints \cite{O_ISAC_001}.
...
Evidence alignment: Coherent receiver implementations explicitly use optical-hybrid/balanced-photodetector style detection chains \cite{O_ISAC_028,O_ISAC_029}. Synchronization and front-end characterization studies further show that coherent receiver interpretation depends on carrier/clock recovery and calibrated electro-optic response estimation in the transceiver chain \cite{O_ISAC_063,O_ISAC_073}. IM/DD receiver implementations explicitly use photodiode-based direct detection and O/E conversion \cite{O_ISAC_001,O_ISAC_023}.
...
Evidence alignment: Representative OW-ISAC studies explicitly use OPA-based beamforming front ends and discuss their role in joint communication-sensing operation \cite{O_ISAC_061,O_ISAC_091}. Phase-time-array and antenna-on-display studies further illustrate how beam steering and spatial scanning become part of the sensing/communication front-end abstraction rather than a separate post-processing layer \cite{O_ISAC_081,O_ISAC_120}.
...
Evidence alignment: Representative photonic ranging studies report bandwidth-limited resolution forms consistent with \(c/(2B)\) under the two-way convention \cite{O_ISAC_026,O_ISAC_034}. Phase-coded FMCW, photonics-assisted radar-ranging, and subcarrier-chirp THz-ISAC waveforms likewise preserve this bandwidth-to-resolution logic in optical or photonics-assisted ranging chains \cite{O_ISAC_012,O_ISAC_152,O_ISAC_159}.
...
Waveform-level coupling is directly shown through waveform-parameter tuning (e.g., power-split control) \cite{O_ISAC_075} and further supported by modulation-index trade-off discussions \cite{O_ISAC_001}. Additional waveform-centric studies optimize low-PAPR OFDM, OCDM super-resolution, and retroreflective multiuser signaling under joint sensing/communication objectives, reinforcing waveform choice as a primary coupling mechanism rather than a secondary implementation detail \cite{O_ISAC_219,O_ISAC_259,O_ISAC_272}.
...
The canonical aggregate PRISMA counts were maintained in a structured screening ledger and reconciled against stage-specific audit records. For the upstream stages, the identification, deduplication, and title/abstract totals were retained as canonical aggregate counts supported by the reconstructed search-stage evidence package; for the later stages, the full-text assessment, exclusion, and final inclusion ledgers were directly row-backed in the repository. In the reconciled record, \textbf{222} full-text articles were assessed, \textbf{2} were excluded at the full-text stage, and the final included corpus comprised \textbf{N = 220} studies. The two reconciled full-text exclusions, together with their study identifiers, exclusion codes, and reasons, were retained in the structured exclusion log and mirrored in the supplementary exclusion appendix prepared for this review. In the present manuscript, the bibliography is explicitly aligned with the full 220-record included set. Most records are used directly in the modality, enabler, and application synthesis, while a small audit-oriented tail is cited here for corpus-traceability completeness rather than weighted as primary technical support in later sections. This traceability tail includes component-level optical papers and adjacent comparator studies retained from the screened set reconstruction \cite{O_ISAC_007,O_ISAC_037,O_ISAC_066,O_ISAC_078,O_ISAC_096,O_ISAC_116,O_ISAC_118,O_ISAC_121,O_ISAC_125,O_ISAC_126,O_ISAC_135,O_ISAC_144,O_ISAC_147,O_ISAC_148,O_ISAC_150,O_ISAC_153,O_ISAC_154,O_ISAC_310,O_ISAC_350,O_ISAC_371}. It also includes a small number of legacy, editorial, or metadata-imperfect records preserved only to keep the reconstructed screening library and repository audit trail internally consistent \cite{O_ISAC_146,O_ISAC_149,O_ISAC_158,O_ISAC_165,O_ISAC_167,O_ISAC_172,O_ISAC_173,O_ISAC_185,O_ISAC_186,O_ISAC_189,O_ISAC_347}. The attrition process is summarised in Fig.~\ref{fig:fig_iii_1}, and the study-oriented ledger for the complete 220-paper corpus was retained in the structured screening archive and mirrored in the supplementary included-studies appendix prepared for this review.
...
FSO covers 19 records (8.6\%), remains mostly shared front-end (14/19), and is mixed in detection with a direct lead (11 direct, 8 coherent). Its task profile is tightly centered on ranging (18/19), so cross-class interpretation should treat FSO as ranging-dominant and observability-mixed rather than extrapolating from either direct-only or coherent-only behavior. Recent records in this class include remote phase-shift LiDAR, retroreflective OFDM/TDD links, and UAV-aided retroreflective platforms, all of which reinforce the class's geometry-conditioned ranging emphasis \cite{O_ISAC_040,O_ISAC_051,O_ISAC_099,O_ISAC_106}. As in the rest of Section IV, direct/coherent outcomes are not merged when metric definitions diverge, and accuracy is not inferred from resolution \cite{O_ISAC_021,O_ISAC_035,O_ISAC_005,O_ISAC_023,O_ISAC_199}.

\subsubsection{Visible Light / LiFi O-ISAC}
VLC/LiFi contributes 25 records (11.3\%), is mostly shared front-end (18/25), and is overwhelmingly direct-detection based (24/25). Its task profile splits between ranging and a strong localization tail, making it the most localization-oriented class among the main wireless media. Visible-light evidence further includes PAM-based joint waveforms, Lambertian calibration, indoor localization services, and camera-assisted optical sensing/communication variants, all consistent with direct-detection and localization-heavy operation \cite{O_ISAC_092,O_ISAC_110,O_ISAC_131,O_ISAC_139}. That emphasis, however, remains tied to intensity-domain observability, so VLC/LiFi comparisons against coherent-leading classes require explicit receiver-model conditioning and preserve the Section II separation between resolution and accuracy \cite{O_ISAC_068,O_ISAC_303,O_ISAC_039,O_ISAC_327,O_ISAC_062,O_ISAC_009}.

\subsubsection{Photonic-THz / Optical-THz Bridging}
Photonic-THz is best read as a bridge class rather than a self-contained medium bucket. The explicit terahertz label appears only once in the corpus, but anchor-level evidence is broader: among 39 papers with direct photonic-THz anchors, 31 are mapped to hybrid labels and only one to explicit terahertz. Anchor evidence also includes photonic-THz system overviews, 60-GHz photonic joint radar-communication, and recent W-band or THz demonstrations that combine ranging with flexible communication, which underscores that the class is operationally photonics-assisted rather than a standalone wireless-only bucket \cite{O_ISAC_002,O_ISAC_057,O_ISAC_301,O_ISAC_356}. For that reason, Section IV uses terahertz as a proxy class while interpreting most photonic-THz system behavior jointly with the hybrid evidence, under the same stage-aware ban on implicit cross-plane substitution \cite{O_ISAC_016,O_ISAC_077,O_ISAC_105,O_ISAC_070,O_ISAC_029}.
...
Recent explicit examples include OFDM-LFM lidar-communication design, polarization-fraction-domain multiplexing through telecom pilots, and photonics-aided LFMCW-style mmWave signaling, all of which make waveform structure the primary coupling interface between sensing and communication \cite{O_ISAC_117,O_ISAC_188,O_ISAC_377}.
...
Representative hardware-centric records include heterogeneous III-V-on-SOI transmitters and monolithic III-nitride chip/circuit realizations, which make the integration claim concrete at device and photonic-platform level rather than only at block-diagram level \cite{O_ISAC_036,O_ISAC_354,O_ISAC_360}.
...
The same control-plane interpretation is visible in low-altitude optical backbone studies, AI-enhanced fiber-wireless mobility support, and confluent edge-network coordination, where integration value is expressed through resource orchestration and network-state adaptation rather than through a single shared transceiver chain \cite{O_ISAC_083,O_ISAC_102,O_ISAC_340}.
...
Recent processing-centric examples include multimodal perception fusion for beamforming, RF-to-LiDAR digital-twin inference, and learning-assisted photonics W-band ISAC reception, which all treat shared inference as a first-class integration layer rather than a downstream add-on \cite{O_ISAC_138,O_ISAC_140,O_ISAC_242}.
...
\begin{equation}
    \Delta r_{\min}\neq \sigma_r
\end{equation}
The first constraint forbids implicit OSNR-to-SNR substitution without an explicit receiver/noise model; the second prevents resolution and accuracy from being treated as aliases. Although structured receiver-detection coverage is high, governance risk remains when SNR-family metrics are not explicitly tied to optical or electrical planes in the narrative. This caution becomes even more important in specialized front ends, where nonstandard receiver physics or radar-centric photonic-THz chains can make generic SNR-style reporting especially opaque without an explicit plane definition \cite{O_ISAC_136,O_ISAC_349}. The contract-violation audit therefore matters directly: 84 flagged records remain in the corpus, split into 75 metric-aliasing cases and 9 measurement-plane ambiguity cases. Section IV-D consequently prioritizes defensible comparability over maximal aggregation, and any statement that mixes planes or interchanges $\Delta r_{\min}$ and $\sigma_r$ is treated as non-comparable evidence \cite{O_ISAC_132,O_ISAC_061,O_ISAC_013,O_ISAC_050,O_ISAC_056}.
...
Joint co-design is required because waveform, beam, power, and ORIS controls share physical constraints. In IM/DD implementations, feasible signaling must satisfy nonnegativity and optical power bounds, while coherent or programmable settings add quantization, steering, and update constraints \cite{O_ISAC_009,O_ISAC_023,O_ISAC_054,O_ISAC_061,O_ISAC_091,O_ISAC_127}. More recent photonics-oriented formulations extend that same shared-control view to beam-pattern-embedded hybrid transceiver optimization, spectrum-fusion cellular base-station integration, and photonics-assisted self-interference cancellation in full-duplex operation \cite{O_ISAC_206,O_ISAC_280,O_ISAC_283}. Model VI-U is useful here because it keeps transmitter, ORIS, and sensing terms in one variable structure.
...
Networked O-ISAC introduces burdens that do not appear in single-link settings: multi-user interference, feedback overhead, sensing-fusion consistency, and coordination delay. The corpus already reports explicit FoV and grating-lobe interference effects in multi-user OPA settings, tracking burden growth with user count in mobile ORIS systems, protocol-level overhead sensitivity in VLC-based networked settings, and newer RO-ISAC/Industry-IoT SDMA extensions that make long-range user separation itself a system-level burden \cite{O_ISAC_009,O_ISAC_061,O_ISAC_091,O_ISAC_098,O_ISAC_218,O_ISAC_303}.
...
VII-A covers outdoor mobility corridors, metro/industrial cabled-fiber corridors, and metropolitan access supervision, where the same optical platform carries traffic while exposing link or environment state for control. Representative cases span weather-conditioned V2V/UAV FSO adaptation, fiber corridors with 1 m vibration monitoring plus 50--60 GBaud 16-QAM carriage, live access supervision with simultaneous event observability and downstream continuity, and submarine/few-mode-fiber infrastructure that combines communication carriage with coastal-event or corridor-state sensing \cite{O_ISAC_003,O_ISAC_005,O_ISAC_020,O_ISAC_038,O_ISAC_064,O_ISAC_069,O_ISAC_074,O_ISAC_248,O_ISAC_276}. Utility-facing fiber studies further suggest that sensing-aware corridor design can coexist with infrastructure functions such as power-over-fiber delivery and wider optical-fiber integration strategy, even when the dual-plane evidence is less complete than in the strongest monitored-corridor exemplars \cite{O_ISAC_014,O_ISAC_348}.
...
\paragraph{Key Takeaways and Research Priorities}
Deployment evaluation in IX-E needs minimum interface contracts binding sensing-positioning and channel-estimation coupling with explicit orchestration handoff checks \cite{O_ISAC_039, O_ISAC_151}. Roadmap claims should use staged readiness gates rather than immediate convergence assumptions \cite{O_ISAC_163}, and governance baselines should require compatibility, model-validity, provenance, and explicit validation boundaries before transferability interpretation \cite{O_ISAC_200}.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/foto_fatih.jpg}}]{Fatih D\"onmez}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/foto_ahmet.jpg}}]{Ahmet Altuncu}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/foto_mustafa.jpg}}]{Mustafa Namdar}
