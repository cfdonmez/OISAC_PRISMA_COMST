...
*   **Çıktı:** `data/proc_markdowns/O_ISAC_XXX/` klasöründe bir `.md` dosyası ve resim klasörü.

👉 **[Detaylı Teknik İnceleme: Phase 1 Nasıl Çalışır?](ph1_digit.md)**
...
👉 **[Detaylı Teknik İnceleme: Phase 2 Nasıl Çalışır?](ph2_vis_analysis.md)**
...
|:---|:---|:---|:---|:---|
| **P1** | `.pdf` | `marker` | `.md` + `.png` | `data/proc_markdowns/` |
| **P2** | `.png` | `Gemini 2.5` | `visual_analysis.txt` | `data/proc_markdowns/.../` |
| **P3** | `.md` + `.txt` | `Llama 3.3` (Groq) | `.json` (+Reasoning) | `data/ext_res_v4/` |
...
*   **Dosya:** [`analysis/nb/extraction_pipeline_v4.py`](analysis/nb/extraction_pipeline_v4.py)
...
*   **Dosya:** [`analysis/nb/extraction_pipeline_v3.py`](analysis/nb/extraction_pipeline_v3.py) (V3'ten miras alındı)
*   **Fonksiyon:** `phase1_marker_conversion()`
*   **Girdi:** `data/ret_docs/[ID].pdf`
*   **Çıktı:** 
    *   `data/proc_markdowns/[ID]/[ID].md` (Ham Metin)
    *   `data/proc_markdowns/[ID]/[ID]/` (Resim Klasörü)

### 3. Faz 2: Görsel Analiz Motoru
*   **Dosya:** [`analysis/nb/extraction_pipeline_v3.py`](analysis/nb/extraction_pipeline_v3.py) (V3'ten miras alındı)
*   **Fonksiyon:** `phase2_visual_analysis()`
*   **Girdi:** Faz 1'den çıkan resimler (`.png`, `.jpg`).
*   **Çıktı:** `data/proc_markdowns/[ID]/visual_analysis.txt` (Grafiklerin metin açıklaması).

### 4. Faz 3: Akıl Yürütme Motoru (CoT Engine)
*   **Dosya:** [`analysis/cot_lab/core/assembler.py`](analysis/cot_lab/core/assembler.py)
*   **Sınıf:** `CoTAssembler`
*   **Görevi:** "Tarif" (Recipe) dosyasına göre LLM'i yönlendirmek.
*   **Kullanılan Tarif:** [`analysis/cot_lab/recipes/experiment_v1_full_analysis.yaml`](analysis/cot_lab/recipes/experiment_v1_full_analysis.yaml)
*   **Girdi:** 
    *   `.md` metni (Faz 1'den)
    *   `visual_analysis.txt` (Faz 2'den)
*   **Çıktı:** [`data/ext_res_v4/[ID]_v4.json`](data/ext_res_v4/) (Nihai Analiz).
...
*   **Nihai Çıktılar:**
    *   `data/ext_res_v4/extraction_v4_unified.json` (Tüm veri seti)
    *   `data/ext_res_v4/extraction_v4_summary.csv` (Excel için özet tablo)
