...
- Table omurgasi taslaga eklendi:
  - `Table V`
  - `Table VI`
  - `Table VII`
...
- `Fig. 4` icin governed operating-cloud / trade-off gorseli henuz uretilmis degil.
- `Fig. 5` icin Pareto/frontier gorseli henuz uretilmis degil.
- Etki: metin gorsellere referans veriyor, ancak final manuskriptte gercek figure asset katmani eksik.

2) Global numbering and cross-reference freeze pending
- `drafts/section_05_correction_notes.md` icinde eski `Fig. 4` cakisimi notu var.
- Bu not final manuskript numbering asamasinda yeniden dogrulanmali; lokal olarak otomatik dogru kabul edilmemeli.
- Etki: Section I ve Section V figure index'leri final dizgide kayabilir.
...
- Frontier/eligibility compression mantigi tabloya yuklenmeyecek; bu yuk `V-C` trade-off metni, Section V closeout synthesis, ve `Fig. 5` caption/text katmaninda kalacak.
...
- `Fig. 4` ve `Fig. 5` caption'lari raw cloud / governed cloud / CRQ-valid / Pareto ayrimini acikca belirtmeli.
...
1. `Fig. 4` ve `Fig. 5` asset'lerini canonical `analysis/V_ev_v2/*.csv` kaynaklarindan uret.
...
Bir sonraki adim:
1. Once `Fig. 4` ve `Fig. 5` icin kisa bir figure specification gecilecek.
...
- `Fig. 4` ve `Fig. 5` gercek asset olarak mevcut olacak.
