...
Section III (Methodology) repo-validated conditionally-freeze-ready duruma getirildi. Section IV section-ici camera-ready polish tamamlandi. Section VI authoritative recovery merge, core hardening, ve integration pass tamamlandi. Section VII icin survey-alignment, reporting-policy lock, ve manuscript-level consistency pass uygulandi. Section VIII editorial closeout ve QA refresh tamamlandi. Tam manuskript integration pass ve release-gate QA da uygulandi. Whole-manuscript figure inventory cikartildi ve section-bazli figure strategy lock notu olusturuldu; aktif manuskript odagi artik final figure numbering/paket karari, Section II figure spec kararleri, Section V legacy denominator uzlasimi, ve gerekiyorsa Section IX conclusion kaynagi kararidir. Abstract ve Section IX final yazimi ise bilerek sonraya birakildi; bunlar figure paketi ve kalan release-gate tutarliliklari kapandiktan sonra skeleton-draft olarak acilacak.
...
- Yeni konusmada en hizli acilis noktasi:
  - ya `Fig. II-1` polish adimlarina devam etmek
...
- Mevcut durum:
  ...
  - Section III icin `Fig. 5` PRISMA anchor/caption aktif, fakat final local asset bu scan'de dogrulanmadi
  - Section IV icin `Fig. IV-1 / Fig. IV-2` specification-level durumda
  - Section V icin `Fig. 4 / Fig. 5` planned durumda; Section III `Fig. 5` ile numbering collision riski acik
...
- Degerlendirme:
  - fig conceptually usable / near-final
  - ana structural issue olan duplicated output box cozuldu
  - kalan is visual polish seviyesinde
- Kalan fix sirası:
  - `Joint design` kutusunu aktiflestir
  - ortak output box'i kompaktlastir
  - output label hizasini toparla
  - sag paneli biraz geri cek
  - basligi hafif kucult
