...
1. `Fig. VI-1` ve `Fig. VI-2` icin gercek figure assetlerini canonical spec'lerden uret.
2. Analysis notebook/pipeline ciktilari tekrar tablo/figure girdisi olarak kullanilacaksa invalid OPA/RIS metric satirlarini filterle veya acikca unvalidated olarak etiketle.
3. Analysis notebook/pipeline tekrar acilacaksa Section 6F sonrasinda readiness rerun yap ve readiness/order mismatch acigini kapat.
4. Final whole-manuscript integration turunda Section I / VII / VIII ile numbering, cross-reference, ve bridge dili kontrolunu yap.
...
- Analysis-level invalid OPA/RIS rows daha sonra publication artefaktlarina tasinirse benchmark ve enabler gorunumu yanlis sertlesebilir.
