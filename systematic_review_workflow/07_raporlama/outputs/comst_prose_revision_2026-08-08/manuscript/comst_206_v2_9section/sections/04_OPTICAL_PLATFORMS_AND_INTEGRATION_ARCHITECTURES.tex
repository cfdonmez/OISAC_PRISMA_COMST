...
corpus shares. Photonic terahertz and fiber together account for 125 studies,
representing 60.7\% of the corpus.

\begin{table}[t]
\caption{Canonical physical organization of the corpus of 206 studies}
...
\midrule
Photonic terahertz & 69 (33.5\%) \\
Fiber & 56 (27.2\%) \\
Visible light communication and LiFi & 38 (18.4\%) \\
...
\subsubsection{Photonic Terahertz Systems}

Photonic terahertz is the largest family, comprising 69 of the 206
...
target path. It yields delay, range, Doppler, angle, imaging, and channel or
target response observables. Communication and sensing are coupled through
hardware, references, waveforms, beams, or resources
...
chain \cite{OISAC_SCR01054}. Comparison across photonic terahertz systems
depends on aligned optical and wireless stages, aperture and beam conditions,
receiver processing, and validation setting.
...
calibration, and wireless geometry. Evidence comes from laboratory links, live
access networks, and a deployed submarine route. For fiber evidence, comparison
depends on the interrogation or transport role, topology, launch and receiver
conditions, conversion and clocking, target path, traffic, and validation setting
\cite{OISAC_SCR00647,OISAC_SCR00038,OISAC_SCR00218,OISAC_SCR00957,
OISAC_SCR00631,OISAC_SCR00496,OISAC_SCR01049}.

\subsubsection{Visible Light Communication and LiFi Systems}

Visible light communication and LiFi comprise 38 studies (18.4\%) and place
communication and sensing within the same illuminated scene. The grouping
preserves their distinct meanings at the link and networking levels. Direct or
reflected room and
...
OISAC_SCR00836,OISAC_SCR00873}. Visible light comparisons depend on matched
task definitions, layout, orientation, illumination, detector, processing, and
...
observations \cite{OISAC_SCR01085,OISAC_SCR00730}. Comparison across free space
optical systems depends on matched geometry, calibration, target properties,
weather, motion, and validation conditions. Hybrid systems extend that comparison across interfaces
...
\cite{OISAC_SCR00210,OISAC_SCR00366,OISAC_SCR00277}. Hybrid comparisons trace
every segment, conversion, clock, and measurement plane together with the
validation of both functions.

Unlike fiber used only for downstream transport, hybrid membership requires
conversion or feedback across segments to shape the result. This occurs when
interaction between fiber and wireless segments is consequential
...
photonic waveform with frequency agility
\cite{OISAC_SCR00091,OISAC_SCR01229}.

\subsubsection{Other Optical Systems}

The three other optical studies (1.5\%) use distinct mechanisms based on optical
control or coherent carriers. These comprise a microwave field generated by a
microcomb, a solar blind ultraviolet path, and a quantum optical path. Their
observables are,
...
Each numerical result is therefore interpreted within the native mechanism and
measurement model of its system. Together, the hybrid and other optical cases
...
Figure~\ref{fig:integration_map} locates seven recurring coupling mechanisms
along a generic signal and service path. A study may occupy several locations,
so the badges report overlapping coverage across the corpus.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/final_submission_2026-08-25/fig05_integration_map.pdf}
\caption{Locations of communication and sensing coupling in the corpus of 206
studies. Equal cards identify hardware, optical carrier, waveform, resource,
link, joint design, and application coupling along the system path. The badges
report overlapping study coverage, with three mixed boundary cases shown
separately. Card position identifies the typical coupling location, while each
card retains equal visual weight.}
\label{fig:integration_map}
\end{figure*}

Resource, hardware, and waveform sharing can form one connected design chain.
Application coupling acts at the service level and carries a different
engineering meaning. The following subsections trace these locations from
physical reuse to coordinated resources and design.
...
\cite{OISAC_SCR00196}. The shared structure is similar, while its physical and
receiver implementation remains specific to the platform.
...
A visible light model for vehicle to everything communication makes exposure
time a shared decision variable.
