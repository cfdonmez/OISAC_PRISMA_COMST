...
def write_outputs(catalog, gap_matrix, table3, borderline, score_dist, demotions):
    ...
    # 3. Table III
    if table3:
        with open(OUTPUT_DIR / "ID_tableIII_coverage_matrix_v2_2.csv", 'w', newline='', encoding='utf-8') as f:
            w = csv.DictWriter(f, fieldnames=table3[0].keys())
            w.writeheader()
            w.writerows(table3)
    
    # 4. Borderline
    with open(OUTPUT_DIR / "borderline_candidates_v2_2.csv", 'w', newline='', encoding='utf-8') as f:
...
def main():
    ...
    v21_borderline = []
    v21_borderline_path = V21_DIR / "borderline_candidates_v2_1.csv"
