...
| Photonic generation / optical-THz bridge | Source and distribution pathway for hybrid optical-wireless control chains | Bridge evidence only; relevant for hybrid system interpretation but not a standalone maturity claim in VI-A | Stage-aware modeling, cross-plane reporting, front-end chain complexity | Connects enabler discussion to hybrid and transfer-oriented system views |

Evidence note: OPA/ORIS prevalence wording follows the strict study-level contribution view used in Section I; broader structured traces from the Section 6F dual-view audit remain contextual and are not interpreted here as direct adoption prevalence.
...
Visual form:
- matrix / heatmap
- rows: normalized medium classes (hybrid, cabled_fibre, wireless_vlc, wireless_fso, residual low-support classes)
...
The benchmark chain in VI-D is also the structural backbone of the section as a whole, and Fig. VI-2 is intended to capture that systems-level coupling from enabler choice to deployment-ready evaluation.
