...
## Author-Governed Claim Support and Missingness Translation

This is an O-ISAC authoring policy adopted on 2026-09-04, not a descriptive
finding from the 76-paper COMST corpus. The full mandatory recipe is
[`OISAC_AUTHOR_GOVERNED_CLAIM_LANGUAGE_RECIPE_2026-09-04.md`](./OISAC_AUTHOR_GOVERNED_CLAIM_LANGUAGE_RECIPE_2026-09-04.md).

Keep the governed evidence layer and the reader-facing narrative distinct.
Internal status labels remain available for audit, while the manuscript leads
with the strongest supported scientific pattern and translates any material
gap into its exact inferential consequence. If a desired numerical relation is
not supported, route the evidence to the strongest truthful lower synthesis
level or to an explicit audit-only terminal route rather than forcing the
relation. Source-level omissions that do not change the claim permitted by the
prespecified J rules need not occupy the main narrative, but claim-changing
missingness and material counterevidence must remain visible beside the claim.
Recurring literature-level gaps must be converted into specific experiment,
benchmark, reporting, or artifact requirements.
