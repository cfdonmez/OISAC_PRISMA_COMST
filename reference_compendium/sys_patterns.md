...
Phase 1: PDF → Markdown
  Input:  data/ret_docs/O_ISAC_XXX.pdf
  Output: data/proc_markdowns/O_ISAC_XXX/O_ISAC_XXX/O_ISAC_XXX.md
  Motor:  extraction_pipeline_v3.py → phase1_marker_conversion()

Phase 2: Visual Analysis
  Input:  data/proc_markdowns/O_ISAC_XXX/.../*.jpg
  Output: data/proc_markdowns/O_ISAC_XXX/.../visual_analysis.txt
...
Phase 5: Agentic Research (Deep Research)
  ...
  Output: analysis/deep_res/output/*_Evidence.md
  Motor:  analysis/deep_res/run_calibration.py → Gemini Agent
```
...
2.  **Eligibility Check:** `protocol/prisma_proto.md` kriterlerine göre çalışmanın soyutunu (abstract) değerlendirir.
3.  **Daily Log Update:** İşlemi `docs/daily_flow.md` (veya günlük loglar) çerçevesinde kayıt altına alır.

### 1. Read-Only Inputs
Kaynak CSV'leri değiştirme. Her zaman yeni output oluştur.

### 2. Protocol-Driven
Her karar `protocol/prisma_proto.md`'ye dayanmalı.

### 3. Checkpoint Kullan
`data/ext_res_v3/checkpoint.json` işlenmiş paper'ları takip eder.
...
| Amaç | Dosya |
|------|-------|
| Ana notebook | `analysis/nb/CoT_Master_Pipeline.ipynb` |
| CoT motoru | `analysis/cot_lab/core/assembler.py` |
| JSON şeması | `analysis/cot_lab/modules/formatting/schema_v2.yaml` |
| PRISMA protokolü | `protocol/prisma_proto.md` |
| Güncel durum | `memory-bank/activeContext.md` |
| Checkpoint | `data/ext_res_v3/checkpoint.json` |
